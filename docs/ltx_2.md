...
- `audio`
  - Audio-only training (uses audio latents + audio prompt embeddings; video branch is disabled)
...
- **Audio datasets**
  - Supported for `--ltx_mode audio`.
  - Requires audio latents + audio prompt embeddings; video latents are dummy placeholders for batching.

Minimal dataset config example (audio-only):
...
## Audio/non-audio batch separation

For mixed datasets (some items have audio, some do not), you can keep audio and non-audio items out of the same batch by adding:

- `--separate_audio_buckets`

This reduces VRAM overhead for image/video-only batches because audio tensors are not allocated unless audio is present.

## Audio-only requirements

Audio-only training uses the audio branch only and expects:

- audio latents cache files (`*_ltx2_audio.safetensors`)
- audio prompt embeddings (from `ltx2_cache_text_encoder_outputs.py --ltx_mode av`)
- dummy video latents generated by `ltx2_cache_latents.py --ltx_mode audio`

Video loss is disabled in audio-only mode; only audio loss is used.
...
For audio-only datasets, use `--ltx_mode audio`. This creates:

- dummy video latents (`*_0001x0001_ltx2.safetensors`)
- audio latents (`*_0001x0001_ltx2_audio.safetensors`)
...
- Optional: `--gemma_safetensors` loads Gemma weights from a single `.safetensors` file (tokenizer/config still come from `--gemma_root`).
...
The audio-only cache step writes dummy video latents. By default, the channel count is inferred from `--ltx2_checkpoint`; override with `--audio_dummy_video_channels` if needed.
...
Audio-only uses the same `--ltx_mode av` cache so the audio prompt embeddings are available.
...
Optional VRAM-saving flag:

- `--sample_with_offloading`
  - Loads text encoder, DiT, and VAE sequentially so only one of them lives on GPU at a time during preview sampling.
- `--sample_disable_audio`
  - Skip audio decoding during preview sampling even in `--ltx_mode av`.
