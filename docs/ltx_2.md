...
| `ValueError: Gemma safetensors is missing required language-model tensors` with `missing_buffers` mentioning `full_attention_inv_freq` or `sliding_attention_inv_freq` | Wrong `transformers` version installed (likely 5.x). Transformers 5.0 renamed Gemma3 rotary embedding buffers (`rotary_emb.inv_freq` → `rotary_emb.full_attention_inv_freq` etc.), which breaks our validation. The safetensors file itself is fine — rotary buffers are runtime-computed, never saved in checkpoints. | `pip install transformers==4.56.1` (our pinned version), or reinstall all deps with `pip install -e .` |
