...
| `--h3_fused_elementwise` | off | Apply each main block's AdaLN modulation (`shift + normalized * (1 + scale)`), its two gated residual adds (`hidden + gate * branch`) and, for adapters whose `multiplier * alpha / rank` is not a power of two, the LoRA delta add as single `addcmul` / `add(alpha=)` kernels. Eager mode only (compiled blocks fuse on their own). The product stays in the fp32 accumulator and is rounded once, so results differ from the default two-kernel form by BF16 rounding; validate before a long run. Precedence with `--h3_fused_indexed_adaln`: when that Triton kernel accepts the layout it handles the norm+modulation itself and the `addcmul` AdaLN path is bypassed for that call; the gated residual and LoRA fusions of this option remain active. Adapters with power-of-two factors (rank 4..128 with alpha 1, 2, 4, ... or `alpha = rank`) take the single-kernel add regardless of this option; it equals the old multiply-then-add for finite values whose scaled magnitude stays out of the subnormal range (verified elementwise on CPU, CUDA coverage pending). |
