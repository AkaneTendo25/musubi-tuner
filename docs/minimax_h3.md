...
- [Dataset](#dataset)
  - [External aligned video guides](#external-aligned-video-guides)
  - [Qwen control visuals](#qwen-control-visuals)
  - [Loss masks](#loss-masks)
  - [Target modalities](#target-modalities)
  - [Reference and target matrix](#reference-and-target-matrix)
...
- [Training a guidance-distilled model](#training-a-guidance-distilled-model)
  - [Methods](#methods)
    ...
    - [Field length floor](#field-length-floor)
...
5. Use [Dataset](#dataset), [Key options](#key-options), and [Memory and speed](#memory-and-speed) as reference material after the basic workflow works. The optional [training dashboard](#training-dashboard) generates and runs these commands from a form; its dataset form covers the common target and source directories, TOML covers the complete modality matrix.
...
| Fixed arbitrary references, video target | [`ref2va.toml`](../examples/minimax_h3/ref2va.toml): `source_*_directory` references; an audio target may carry references too | Ref2VA | `ref2va` | `--h3_training_mode ref2va` |
| Fixed arbitrary references, image target | [`image_ref2va.toml`](../examples/minimax_h3/image_ref2va.toml) | Ref2VA | `ref2va` | `--h3_training_mode ref2va` |
| References plus target-derived timeline guides | [`ref2va_guides.toml`](../examples/minimax_h3/ref2va_guides.toml) | Ref2VA | `ref2va` | `--h3_training_mode ref2va`; `--h3_guide_specs` or target-derived keyframes |
| References plus an external aligned video guide | [`ref2va_aligned_guide.toml`](../examples/minimax_h3/ref2va_aligned_guide.toml) | Ref2VA | `ref2va` | `--h3_training_mode ref2va`; `aligned_guide_indices` |
...
Python 3.10–3.12. All files come from [Comfy-Org/MiniMax-H3](https://huggingface.co/Comfy-Org/MiniMax-H3):
...
The image-editing rows use separate basename-matched source images and require the shown `--h3_image_mode` on both cache
commands. The video `i2va`, `fl2va`, and `l2va` rows take their keyframes from the target itself; they are not Ref2VA
reference directories. Train on the cache made with the selected task; a mismatched cache is rejected.
...
This example uses the optional pre-quantized conditioner from [Model download](#model-download).

- With only the BF16 conditioner, `--text_encoder_quantization nvfp4` quantizes its Linear weights to block-16 NVFP4 while
  loading. Activations and GEMMs stay BF16, so this W4A16 path also works on pre-Blackwell CUDA GPUs.
- `--h3_text_encoder_blocks_to_stream 50` streams all 50 frozen language blocks and minimizes their GPU residency. Smaller
  values cost less transfer and keep more blocks resident. Streaming works with BF16 and both NVFP4 modes, not with the
  bitsandbytes `int8` or `nf4` loaders.
- If the full BF16 conditioner fits, use `--text_encoder_quantization none` without streaming.
- Add `--cache_guidance_empty` if training will use caption dropout or guidance distillation.
...
The example swaps 40 of the 50 main blocks.
...
“Yes” means the cache and packed training layout support the combination; it does not imply that every combination was
represented in the released checkpoint.

The released target canvas is a 768-pixel short edge with a 1344×768 area cap; reference videos are sized the same way
(`--reference_video_short_edge`, `--reference_video_max_pixels`), reference images default to a 2048-pixel short edge
(`--reference_image_short_edge`). Other 32-pixel-aligned sizes work but sit outside the released canvas distribution.

## Pre-caching

Both caches are required before training; the trainer never loads a VAE or the conditioner to rebuild a missing one. The two
commands, the `--vae`/`--audio_vae` rules and `--skip_existing` are in [Cache FL2VA latents](#cache-fl2va-latents) and
[Cache FL2VA text embeddings](#cache-fl2va-text-embeddings). This section covers the remaining cache options.

`--task` must match how you intend to train: `t2va` (text only), `i2va` (first frame), `fl2va` (first+last), `l2va`
(last frame), `ref2va`, or `ref2va_omni`. Keyframe tasks take their frames from the target video itself, not from control fields; the exception is conditioned-image
mode (`--h3_image_mode`, below), where the endpoints come from the control images.
...
Conditioner memory: `--text_encoder_quantization int8`, `nf4` or `nvfp4` quantizes the BF16 checkpoint while loading
(`nvfp4` is calibration-free block-16 W4A16 with BF16 activations); `nvfp4_awq` loads the released pre-quantized file. All
of them reduce GPU residency only: the BF16 checkpoint is still memory-mapped into host address space while weights are
converted. `--h3_text_encoder_blocks_to_stream N` keeps `N` of the 50 frozen language layers in CPU memory and moves them
through a two-layer GPU ring during encoding; requires CUDA, works with BF16, `nvfp4` and `nvfp4_awq`, not with the
bitsandbytes `int8` or `nf4` loaders. The workflow sections show the combination in use.
...
Reference sections first, then the specialised trainers: learned context, slider, full-parameter. Training on the
guidance-distilled checkpoints is a section of its own below.

### Start LoRA training

The LoRA command and its block-swap options are in [Start FL2VA training](#start-fl2va-training) and
[Start Ref2VA training](#start-ref2va-training). The sections below apply to both.
...
It adapts the rate itself, so `--learning_rate` is a starting
...
calibration stay frozen. Named targeting selects modules without regular expressions:
...
| --- | --- | --- |
| `--sdpa`, `--flash_attn`, `--flash3`, `--flash4` | required | Select one attention backend. SDPA needs no additional package; FlashAttention requires its corresponding installation and compatible GPU. Unsupported padded batches fall back to SDPA. |
| `--h3_attn_auto_dispatch` | off | Prefer cuDNN SDPA for large maskless workloads. Requires `--sdpa`. Changes rounding; benchmark first. |
| `--h3_attn_autotune` | off | Time every runnable unmasked backend — plain SDPA, cuDNN-priority SDPA, and each installed FlashAttention — on the real packed Q/K/V once per shape bucket, then use the measured winner. Requires `--sdpa`; redundant with `--h3_attn_auto_dispatch` (the probe subsumes its fixed priority). Masked, block-sparse, and INT8 calls keep their existing paths. Changes which kernel runs, so it changes rounding; the winner is logged once per bucket. |
...
| `--h3_null_anchor_reuse_empty` | off | Reuse the guidance step's frozen empty-prompt prediction as the null anchor's frozen reference instead of running a second identical forward: one less transformer pass per anchored prompted step. Requires `--h3_guidance_distillation_scale`, `--h3_guidance_null_source frozen`, and `--h3_guidance_null_anchor_weight` above 0; silently inert otherwise. The reference shares the guidance branch's conditioning-noise draw rather than drawing its own — both are samples of the same 0.001-sigma conditioning distribution. |
| `--h3_null_anchor_shared_draws` | off | Replay the anchored empty-prompt student's entry RNG for the frozen null-anchor reference, so the pair describes one conditioning-noise sample — the same discipline the rollout pairs already use. Without it the reference draws the continuation of the student's stream. Requires an active `--h3_guidance_null_anchor_weight`; unused while `--h3_null_anchor_reuse_empty` supplies the reference. |
...
| `--h3_gradient_checkpointing_swapped` | off | Under `--blocks_to_swap`, checkpoint exactly the blocks whose weights are streamed and leave the resident blocks eager, skipping their forward recomputation at backward. Requires `--gradient_checkpointing` and `--blocks_to_swap`; mutually exclusive with `--h3_gradient_checkpointing_blocks`. Each resident-eager block retains its activations, so the step-time win is paid in VRAM. |
...
**`--h3_compile_attention`.** `opaque` wraps the kernel call in `torch.compiler.disable`, so each block compiles to exactly one graph before the kernel and one after it; use it when the compile log shows graph breaks inside the attention call (a FlashAttention binding without `torch.library` fake kernels), which otherwise cascade into small unfusable fragments around every block's kernel. `auto` chooses `opaque` for `--flash_attn`/`--flash3` and `inline` for `--sdpa`. Not combinable with `--compile_fullgraph`, which forbids any graph break; `auto` falls back to `inline` there. Numerics are unchanged: the wrapped call is the same eager kernel and autograd.
...
The sparse auxiliary objectives (rollout selection and stop sigma, sparse preservation, DOP, guidance probability) draw
...
| `--h3_convrot_int8_fwd bf16` | Default `int8`. `bf16` evaluates the matmul in BF16 without changing stored weights; usually faster on GPUs with fast BF16, measure both. |
| `--h3_convrot_int8_bwd int8` | Default `bf16`. INT8 input-gradient path; incompatible with `--h3_convrot_int8_fwd bf16`, which leaves no rotated activations. With `--h3_convrot_int8` it requires Triton; on a pre-quantized `--int8_convrot_base` it falls back to `torch._int_mm` and then to a dense matmul. |
...
**`--h3_checkpoint_keep`.** `attention` retains one rows-by-hidden BF16 tensor per checkpointed block; `qkv` retains four. `adaln` additionally retains modulation outputs to avoid their projection recompute. `mlp` additionally retains each block's `mlp.fc1` output (rows-by-2x-ffn — the largest single activation in the block), skipping the biggest remaining matmul in the recompute. Weights are never kept, so whole-block H2D-only block swap remains available. Attention reuse needs a fused attention kernel; SDPA math fallback raises an error. Rejected with `--gradient_checkpointing_cpu_offload`, `--compile`, `--h3_int8_attention train`, block-sparse attention, `--block_swap_granularity layer`, and (`qkv`, `adaln` or `mlp`) `--h3_convrot_int8_lora_fused`.
...
loss weight is forced to zero, so only the trained modality receives direct supervision; the two modalities still share
H3's attention parameters.
...
Every objective here is off unless its flag is given; a run that names none of them trains the plain objective.
...
| `--h3_rollout_fused_teacher` | Compute the student and teacher predictions for one supervised state in a single pass over the blocks, so each swapped block is streamed once per state instead of once per arm. May increase transient memory. `--h3_int8_attention aux` composes with it -- each frozen arm's own context selects the INT8 kernel around its block calls while the student stays dense. Rejected together with block-sparse attention (`--h3_block_sparse_kv_fraction` or `--h3_block_sparse_threshold` above `0`), whose per-sequence attention plan the fused pass cannot keep separate per arm. |
...
| `--h3_two_teacher_weights teacher.safetensors` | Two-teacher distillation: a frozen second adapter, live only on the concept teacher's own forward. See [Two-teacher distillation](#two-teacher-distillation). |
| `--h3_two_teacher_loss_weight 0.0` | Weight of the distillation objective; `0` (default) disables it. On active steps the adapter is matched to the concept teacher under the trigger and to the untouched base under the trigger-free caption, at the same state. Two no-grad forwards and one gradient-carrying forward beyond the ordinary step. |
...
| `--h3_two_teacher_snapshot_step 0` | Run both phases in one command: train the concept with the ordinary objective for this many steps, then freeze the adapter in memory as the teacher and distil for the rest of the run. `0` (default) is off. Rejected together with `--h3_two_teacher_weights`. |
...
python examples/minimax_h3/learned_context/generate_bootstrap_targets.py \
  ...
  --model /path/to/minimax_h3_fl2va_pruned_int8_convrot.safetensors --int8_convrot_base \
...
`--h3_learned_context_composition prepend` retains each cached caption; `replace` drops it and trains a self-contained
context. The saved file contains the optimized Qwen rows under
...
intentional. The text null presentation is encoded in the same up-front pass, with the same text-row layout. `--h3_fuse_frozen_teachers` is not
...
The released H3 checkpoints are guidance-distilled: one prompted forward returns what an undistilled model returns
with classifier-free guidance, the empty-prompt prediction plus a scale times the prompted-minus-empty difference.
...
Each step also evaluates the model with the empty prompt. The target is the empty-prompt prediction plus S times the
offset from it to the data velocity. The empty-prompt prediction is the student's own and moves during training.
...
The student runs its own sampler a few steps from noise. At the resulting states it is supervised against the frozen
checkpoint given information the student does not receive: the clip's reference or keyframes, or a richer caption.
This is the on-policy objective of D-OPSD. It runs on the fraction of steps set by `--h3_rollout_probability`; the other
steps run the plain data loss, or the guidance target when one is configured.

- **Hybrid D-OPSD (H-OPSD):** rollout supervision on half the steps, the contrastive target on the other half.
  Learns a concept at the base model's output quality. The concept is unconditional: it also appears on prompts
  without the trigger word.
- **Applies to:** both checkpoints; the privilege differs. Ref2VA: `--h3_rollout_teacher_privilege reference`, the
  teacher gets more references than the student (frames of the target itself). The teacher forward runs over the
  reference rows, so rollout steps 2 and window 1 keep the cost down. FL2VA: `keyframe`, the teacher gets the clip's
  first and last frames, the student trains text-only (`t2va` cache); rollout steps 4 and window 2 are affordable.
  `caption` is a privilege only when the teacher caption carries information the student's does not.
- **Advantages:** supervises the states the sampler visits; corrects field direction; the teacher is the same
  checkpoint.
- **Disadvantages:** a second cached dataset for the teacher. The teacher must receive information the student does
  not; the trainer rejects a teacher whose conditioning the student also has. The teacher predicts with less
  amplification than the checkpoint, so the field still shortens unless the floor is added. Cost **1.9x** on FL2VA,
  **5x to 7x** on Ref2VA, where the teacher input includes reference frames.
...
At the rollout states the student's field length is compared with the frozen checkpoint's at the same state. The
step is penalised when the student's is shorter. By default (`--h3_rollout_field_floor_direction self`) the floor does
not act on the direction; `teacher` floors the projection onto the teacher's field and turns the field as well. The floor
is measured against the frozen empty branch, so it goes together with the null anchor.
...
- **Flags:** `--h3_rollout_field_floor 0.3` (requires `--h3_rollout_supervision` and a text cache with
  `--cache_guidance_empty`; without a null anchor the trainer warns that the floor bounds the prompted branch only); optionally
  `--h3_rollout_field_floor_sigma_max`, `--h3_rollout_field_floor_sigma_min`, `--h3_rollout_field_floor_direction {self,teacher}`.
- **Direction-only teacher:** `--h3_rollout_teacher_magnitude_weight 0` makes the rollout term match the teacher's
  ...
  reference-privileged teacher (Ref2VA) it keeps a longer field at the same direction; part of that recipe. With a
  keyframe teacher and a trigger word (FL2VA) it suppresses the concept: the teacher's direction is the base's, and
  following it alone leaves nothing to learn. Do not use it there. Upstream musubi-tuner applies the same split to
  its off-policy teacher-matching loss (kohya-ss/musubi-tuner#1065, #1086).
...
- `--h3_adapter_prompt_only` runs every empty-prompt forward with the adapter disabled, so the student's empty branch is
  the frozen one by construction and the anchor's forwards are saved. Rejected together with either null anchor weight
  and with `--h3_caption_dropout_rate` above 0. Weaker than the anchor: the shared weights still drift.
- `--h3_guidance_scale_sigma_max 0.9` disables the target amplification above that sigma, where the implied scale is
  very large and the data is close to noise. Better validation fit; combine with the floor.
...
- `--h3_validation_multipliers 0.75,1.25` repeats the data-state validation at each listed LoRA multiplier and reports
  `val/m<multiplier>/velocity_err_rel`, `field`, `field_cos` and `drift/prompted_rel`. One extra pass per multiplier; the
  rollout probe is not repeated. LoRA runs only.
- `--h3_validation_bare_dataset_config bare.toml`: the trigger probe. A second validation set over the same items
  ...
  the concept is conditional. LoRA runs only; doubles the probe cost.
- Diagnostics: `--h3_term_grad_every`, `--h3_adapter_stats_every`, `--h3_train_sigma_bins`, `--h3_validation_std`; see
  [Auxiliary objectives](#auxiliary-objectives).

### Metrics

The training loss does not show the loss of amplification. Three validation metrics do. They are computed on the
...
Training metrics are enabled by the flags listed. Everything under `val/` needs `--validation_dataset_config`; the
field metrics need `--h3_validation_field_probe` or `--h3_validation_rollout_probe`.
...
| `val/trigger/leak_share`, `val/trigger/leak_cos` (+ `/bin{k}`) | bare dataset config | The adapter's move on the bare caption projected onto its move with the trigger at the same item and state: the share of the trigger's update the bare prompt received, and the cosine between the two moves. Pairs only items at the same position in both sets; `val/trigger/leak_pairs` counts the paired item-and-sigma-bin states. | share near 0 for a conditional concept; near 1 when the same update lands on every prompt |
...
--optimizer_type adamw8bit --learning_rate 1e-4 --timestep_sampling sigmoid \
...
**Ref2VA: NAFP, direction-only teacher.** 3.5x to 4x a plain step. Converges in fewer steps than a plain LoRA;
later checkpoints lose field direction without improving the fit. `--h3_rollout_teacher_magnitude_weight 0` keeps
a longer field. `--h3_guidance_null_anchor_probability` below 1 saves step time but weakens the concept; keep it
at 1.
...
Cheaper: H-OPSD with the same teacher, about 3x a plain step. Same output quality, weaker concept, less complete
transformation of the subject, unconditional.
...
**FL2VA, concept (e.g. a style) on every prompt: H-OPSD.** About 2x a plain step. Full concept, base output
quality, shorter field; the concept also shows up on prompts without the trigger. Teacher config: keyframe task
(`--task fl2va` text cache). Student: text only (`t2va`).
...
```shell
accelerate launch minimax_h3_train_network.py ... \
  --dit minimax_h3_fl2va_bf16.safetensors --max_train_steps 2000 \
  --h3_dop_trigger "xyz style" --h3_dop_caption_mode bare \
...
> [!WARNING]
> Review every generated command before starting a long or expensive job.

The dashboard generates and runs H3 caching, training, and inference commands, and shows live output, metrics, and samples.
Contributed by [@Ada123-a](https://github.com/Ada123-a). The optimizer **Set** button fills in the Automagic3 values from
[Optimizers](#optimizers).
