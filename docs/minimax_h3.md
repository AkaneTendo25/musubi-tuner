...
- `--num_shards N --shard_index I` runs deterministic shard `I` (`0 <= I < N`). Use the same pair for latent and text caching and run every index to complete the cache.
...
| `--h3_attn_autotune` | off | Time available unmasked attention backends once per exact shape and use the fastest. Requires `--sdpa`; incompatible with `--h3_attn_auto_dispatch`. Masked, block-sparse, and INT8 calls are unchanged. Kernel choice can change rounding. |
...
| `--h3_lora_fused_bf16` | off | Fold the LoRA delta add into eligible bias-free BF16 Linear GEMM epilogues. Dropout, split, biased, quantized, non-BF16, NoRA-forward, and compiled calls use the regular path. BF16 reassociation can change rounding; measure quality and speed before a long run. |
| `--h3_varlen_padding` | off | Pack key-validity-padded batches into one FlashAttention variable-length call; falls back for unsupported devices or dtypes, unavailable FlashAttention, and pairwise masks. |
| `--h3_sample_keep_dit_resident` | off | Keep the DiT on GPU during training-sample decoding to avoid moving its weights to CPU and back. Needs enough VRAM for the decoder beside the model; decode OOM retries with the normal evacuation path. |
...
| `--h3_reusable_activation_offload` | off | Reuse pinned CPU checkpoint buffers and prefetch them for backward. Requires `--gradient_checkpointing --gradient_checkpointing_cpu_offload` and sufficient system RAM. |
...
| `--h3_gradient_checkpointing_swapped` | off | Checkpoint only streamed blocks and leave resident blocks eager. Requires gradient checkpointing and block swap; incompatible with `--h3_gradient_checkpointing_blocks`. Uses more VRAM to reduce recomputation. |
