...
def convrot_int8_linear_forward_patch(self: nn.Linear, x):
    if self.weight.device != x.device:
        self.weight.data = self.weight.data.to(x.device, non_blocking=True)
    if self.scale_weight.device != x.device:
        self.scale_weight = self.scale_weight.to(x.device, non_blocking=True)
    return ConvRotInt8LinearFn.apply(
        x, self.weight, self.scale_weight, self.bias, self._convrot_groupsize, self._convrot_bwd_mode
    )


def apply_convrot_int8_monkey_patch(model, optimized_state_dict, bwd_mode: str = "native", groupsize: int = CONVROT_GROUPSIZE):
    """
    ...
    Args:
        ...
        bwd_mode (str): "native" (dequantizes to the gradient dtype) or "int8" (quantizes
    ...
    """
    if bwd_mode not in ("native", "int8"):
