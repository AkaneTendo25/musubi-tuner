...
class MaskInfo:
    """Routing state for one TREAD window."""
...
class TREADRouter:
    ...
    def _importance(x: torch.Tensor) -> torch.Tensor:
        mags = x.abs().sum(-1)
        mins = mags.min(dim=1, keepdim=True)[0]
        rng = mags.max(dim=1, keepdim=True)[0] - mins
        return (mags - mins) / (rng + 1e-8)
        ...
        *,
        ...
        if force_keep.shape[0] == 1 and batch_size != 1 and force_keep.shape[1] == seq_len:
            ...
            if force_keep.numel() == batch_size * seq_len:
                ...
                raise ValueError(
                    f"force_keep mask has shape {tuple(force_keep.shape)}, expected ({batch_size}, {seq_len})"
                )
        return force_keep
    ...
    def get_mask(
        ...
        num_force = force_keep.sum(dim=1)
        base_keep = seq_len - int(round(seq_len * float(mask_ratio)))
        keep_budget = max(base_keep, int(num_force.max().item()) if num_force.numel() > 0 else 0, 1)
    ...
    def start_route(self, x: torch.Tensor, info: MaskInfo) -> torch.Tensor:
        ...
        x_shuf = torch.take_along_dim(x, gather_idx, dim=1)
    ...
    def end_route(
        ...
        x_shuf = torch.empty(batch_size, seq_len, hidden_dim, device=routed_x.device, dtype=routed_x.dtype)
        ...
        gather_idx = ids_restore.unsqueeze(-1).expand_as(x_shuf)
        return torch.take_along_dim(x_shuf, gather_idx, dim=1)


def default_ltx_tread_route(ltx_version: str) -> dict[str, Any]:
    """Return the default TREAD route window for LTX training."""
    route: dict[str, Any] = {"selection_ratio": 0.5}
...
def parse_tread_args(
    ...
    *,
    ...
    """Parse musubi's single-route `--tread key=value ...` CLI form."""
    ...
    for item in items:
        if "=" not in item:
            raise ValueError(f"--tread expects key=value items, got: {item!r}")
        raw_key, raw_value = item.split("=", 1)
        key = key_aliases.get(raw_key.strip())
        if key is None:
            raise ValueError(
                f"Unknown --tread option {raw_key!r}. "
                "Supported keys: selection_ratio, start_layer_idx, end_layer_idx."
            )
