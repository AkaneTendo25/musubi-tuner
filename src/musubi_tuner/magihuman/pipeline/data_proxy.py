...
try:
    ...
    class UnfoldNd:
        ...
        def __call__(self, x: torch.Tensor) -> torch.Tensor:
            ...
            patches = patches.contiguous().permute(0, 1, 5, 6, 7, 2, 3, 4)
            return patches.reshape(x.shape[0], x.shape[1] * kt * kh * kw, -1)

if TYPE_CHECKING:
    from .video_generate import EvalInput
...
class SingleData:
    ...
    def coords_mapping(self):
        if self.spatial_rope_interpolation == "inter":
            video_ref_feat_shape = (self.padded_t // self.t_patch_size, 32, 32)
        else:
            video_ref_feat_shape = (
                self.padded_t // self.t_patch_size,
                self.padded_h // self.patch_size,
                self.padded_w // self.patch_size,
            )

        video_coords = self.default_coords(
            shape=(self.padded_t // self.t_patch_size, self.padded_h // self.patch_size, self.padded_w // self.patch_size),
            ref_feat_shape=video_ref_feat_shape,
        )

        if self.coords_style == "v1":
            audio_coords = self.default_coords(
                shape=(self.audio_feat_len, 1, 1), ref_feat_shape=(self.padded_t // self.t_patch_size, 1, 1)
            )
    ...
    def depack_token_sequence(self, token_sequence):
        ...
        video_x_t = rearrange(
            ...
            H=self.padded_h // self.patch_size,
            W=self.padded_w // self.patch_size,
            pT=self.t_patch_size,
            pH=self.patch_size,
            pW=self.patch_size,
        ).contiguous()[:, : self.orig_t, : self.orig_h, : self.orig_w]
...
class MagiDataProxy:
    ...
    def _pad_video_to_patch_grid(self, x_t: torch.Tensor):
        _, _, t, h, w = x_t.shape
        pad_t = (-t) % self.t_patch_size
        pad_h = (-h) % self.patch_size
        pad_w = (-w) % self.patch_size
        if pad_t == 0 and pad_h == 0 and pad_w == 0:
            return x_t, (t, h, w), (t, h, w)

        padded = F.pad(x_t, (0, pad_w, 0, pad_h, 0, pad_t), mode="constant", value=0.0)
        return padded, (t, h, w), (t + pad_t, h + pad_h, w + pad_w)

    def process_input(self, transported_data: "EvalInput"):
        # init img2col module

        x_t_padded, orig_shape, padded_shape = self._pad_video_to_patch_grid(transported_data.x_t)
        batch_size, _, _, _, _ = x_t_padded.shape
        # 1. Process video features while keeping the batch dimension
        x_t = self.img2tokens(x_t_padded)
