"""Automagic v3 optimizer ported from AI Toolkit.

Source: https://github.com/ostris/ai-toolkit/blob/4e91fb2d0a548a29f82cb905a32a0156715edb44/toolkit/optimizers/automagic3.py
Copyright (c) 2024 Ostris, LLC. Licensed under the MIT License; see
``AI_TOOLKIT_LICENSE.txt`` in this package.
...
class Automagic3(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        fused: bool = True,
        stochastic_grad_accumulation: bool = True,
    ):
        if min_lr > max_lr:
        ...
        self.fused = fused
        # Musubi's trainer owns accumulation, distributed reduction, and
        # clipping, so native integrations can disable this upstream hook.
        self.stochastic_grad_accumulation = stochastic_grad_accumulation
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if self.fused:
                    ...
                    handle = p.register_post_accumulate_grad_hook(self._make_backward_hook(group))
                    self._hook_handles.append(handle)
                elif p.dtype != torch.float32 and stochastic_grad_accumulation:
                    ...
                    handle = p.register_post_accumulate_grad_hook(self._make_accum_hook())
                    self._hook_handles.append(handle)
    ...
    def _pack_consts(cls, device):
        ...
        if consts is None:
            consts = (
                torch.tensor([1, 2, 4, 8, 16, 32, 64, 128], device=device, dtype=torch.uint8),
                torch.tensor([0, 1, 2, 3, 4, 5, 6, 7], device=device, dtype=torch.uint8),
            )
    ...
    def _rebuild_group_index(self) -> None:
        ...
        self._param_group_index = {p: gi for gi, group in enumerate(self.param_groups) for p in group["params"]}
    ...
    def _init_state(self, p: torch.Tensor, group: dict) -> None:
        ...
        width = (p.numel() + 7) // 8
        state["sign_history"] = torch.zeros((H, width), dtype=torch.uint8, device=p.device)
    ...
    def _update_param(self, p: torch.Tensor, group: dict) -> None:
        ...
        if fill == H:
            ...
            bits = (chron.unsqueeze(-1) >> shifts).bitwise_and_(1).view(H, -1)[:, : update.numel()]
    ...
    def get_learning_rates(self) -> List[float]:
        ...
        out = []
        for group in self.param_groups:
            lrs = [self.state[p]["lr"] for p in group["params"] if p in self.state and "lr" in self.state[p]]
    ...
    def load_state_dict(self, state_dict):
        ...
        for group in self.param_groups:
            ...
            lrs = [
                st["lr"]
                for p in group["params"]
                if (st := self.state.get(p)) is not None and isinstance(st.get("lr"), torch.Tensor)
            ]
            med = None
            if lrs:
                dev = lrs[0].device
                med = torch.stack([t.to(torch.float32).to(dev) for t in lrs]).log_().median().exp_()
            for p in group["params"]:
                st = self.state.get(p)
                if st is None:
                    continue
                ...
                if hist_ok:
                    ...
                    st["sign_history"] = torch.zeros((H, width), dtype=torch.uint8, device=p.device)
                    st["hist_idx"] = 0
                    st["hist_fill"] = 0
