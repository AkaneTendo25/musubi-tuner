...
class LTX2SamplingMixin:
    ...
    def do_inference(
        ...
        audio_ref_ic_sampling = (
            resolved_ic_strategy == "audio_ref_ic" and self._ltx_mode in {"av", "audio"} and ref_audio_latents is not None
        )
        av_ic_sampling = (
            resolved_ic_strategy == "av_ic"
            and self._ltx_mode == "av"
        ...
        if sampler_name == "res_2s" and (
            audio_latents is not None or audio_ref_ic_sampling or av_ic_sampling or video_ref_only_av_sampling
        ):
        ...
        with torch.no_grad():
            for step_idx in tqdm(range(len(sigmas) - 1), desc="LTX-2 preview", leave=False):
                ...
                if audio_ref_ic_sampling and audio_model_input is not None and ref_audio_seq_len > 0:
