    ...
    def terminate(self):
        ...
        if not process_refs:
            ...
            with self._lock:
                ...
                force_stop = self._force_stop_requested
                graceful_stop_requested = not force_stop and self._request_graceful_training_stop()
        else:
        ...
        if force_stop:
            ...
            with self._lock:
                self._stop_process_refs = list(process_refs)
            t = threading.Thread(target=self._force_kill, args=(process_refs, 0), daemon=True)
            t.start()
            return

        if graceful_stop_requested:
            self._force_kill_delay_seconds = 600
            t = threading.Thread(target=self._force_kill, args=(process_refs, 600), daemon=True)
            t.start()
            return
        ...
        t = threading.Thread(target=self._force_kill, args=(process_refs, self._force_kill_delay_seconds), daemon=True)
        t.start()

    def _force_kill(self, process_refs: list[ProcessRef], delay_seconds: float):
        if self._proc:
            try:
                self._proc.wait(timeout=delay_seconds)
