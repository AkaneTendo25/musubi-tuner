<script>
	...
	let { entry = $bindable({}), index = 0, onRemove } = $props();
...
<div class="overflow-hidden" style="background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); box-shadow: var(--shadow-sm);">
	...
	<div class="p-4 space-y-3">
		...
		<PathInput label="{isAudio ? 'Audio' : isVideo ? 'Video' : 'Image'} Directory" bind:value={entry.directory} tooltip="Directory containing media files" />
		<PathInput label="JSONL File" bind:value={entry.jsonl_file} showFiles placeholder="Optional (alternative to directory)" tooltip="JSONL file listing individual media paths instead of a directory" />
		<PathInput label="Cache Directory" bind:value={entry.cache_directory} tooltip="Directory to store cached latents/embeddings" />
		<PathInput label="Reference Cache Dir" bind:value={entry.reference_cache_directory} placeholder="Optional" tooltip="Reference cache directory for I2V / control workflows" />
		{#if !isAudio}
			<PathInput label="Control Directory" bind:value={entry.control_directory} placeholder="Optional" tooltip="Directory with control images/videos (e.g. depth maps, edges)" />
		{/if}
