<script>
	...
	const navItems = [
		...
		{ href: '/samples', label: 'Samples', group: 'Tools', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', ltxOnly: true },
		{ href: '/training/conditioning', label: 'Conditioning', group: 'Workflow', icon: 'M12 2l9 5-9 5-9-5 9-5zM3 12l9 5 9-5M3 17l9 5 9-5', ltxOnly: true },
		...
		{ href: '/training/techniques', label: 'Methods', group: 'Advanced', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z', advancedOnly: true, ltxOnly: true },
		{ href: '/training/full-finetune', label: 'Fine-tuning', group: 'Advanced', icon: 'M4 4h16v16H4z M8 8h8 M8 12h8 M8 16h5', processTypes: ['full_finetune'], advancedOnly: true, ltxOnly: true },
		{ href: '/training/rl', label: 'RL Post-Training', group: 'Advanced', icon: 'M12 21a9 9 0 100-18 9 9 0 000 18z M12 15a3 3 0 100-6 3 3 0 000 6z', processTypes: ['rl_cache_rollouts', 'rl_train'], advancedOnly: true, ltxOnly: true },
	...
	function navLabel(item) {
		if (item.href === '/') return '01  Overview';
		if (item.href === '/dataset') return '02  Dataset';
		if (item.href === '/caching') return '03  Caching';
		if (item.href === '/training') return '04  Training';
		if (item.href === '/training/dashboard') return '05  Monitor';
		if (item.href === '/inference') return '06  Inference';
		return item.label;
	}

</script>

<nav class="h3-sidebar w-52 flex-shrink-0 flex flex-col h-screen sticky top-0" style="background: var(--sidebar-bg); border-right: 1px solid var(--border-subtle);">
	<!-- Logo -->
	<div class="h-12 px-3 flex items-center" style="border-bottom: 1px solid var(--border-subtle);">
		<div class="flex items-center gap-2 min-w-0 w-full">
			<div class="w-7 h-7 flex items-center justify-center flex-shrink-0" style="background: var(--accent); box-shadow: var(--logo-shadow); border-radius: var(--logo-radius); clip-path: var(--logo-clip);">
				<span class="text-[9px] font-bold" style="color: var(--bg-base);">H3</span>
			</div>
	...
	<div class="flex-1 py-2 px-2 space-y-3 overflow-y-auto">
		{#each navGroups as group}
			{@const visibleItems = navItems.filter((item) => navGroup(item) === group && (item.always || $projectLoaded) && (!item.advancedOnly || $uiMode === 'advanced') && !(isH3 && item.ltxOnly))}
