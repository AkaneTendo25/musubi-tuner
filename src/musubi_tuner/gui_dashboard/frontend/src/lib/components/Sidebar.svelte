<script>
	...
	const navItems = [
		...
		{ href: '/training/techniques', label: 'Techniques', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z', processTypes: ['slider_training'], advancedOnly: true },
...
<nav class="w-52 flex-shrink-0 flex flex-col h-screen sticky top-0" style="background: var(--sidebar-bg); border-right: 1px solid var(--border-subtle);">
	...
	<div class="flex-1 py-2 px-2 space-y-0.5 overflow-y-auto">
		{#each navItems as item}
			...
			{@const visible = (item.always || $projectLoaded) && (!item.advancedOnly || $uiMode === 'advanced')}
			{#if visible}
				<a
					href={item.href}
