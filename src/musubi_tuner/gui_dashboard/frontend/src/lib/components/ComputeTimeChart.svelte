<script>
	...
	import { stepTimeData, dataWaitData } from '../stores/metrics.js';
	...
	function baseOption() {
		return {
			...
			tooltip: {
				...
				formatter: (params) => {
					const p = params[0];
					return `Step ${p.value[0]}<br/>${p.value[1].toFixed(2)}s compute`;
				}
			},
			xAxis: {
				type: 'value',
	...
	function buildComputeRows(stepRows, waitRows) {
		const waitByStep = new Map(waitRows.map((r) => [r.step, r.data_wait_time ?? 0]));
		return stepRows.map((r) => ({
			step: r.step,
			compute_time: Math.max((r.step_time ?? 0) - (waitByStep.get(r.step) ?? 0), 0)
		}));
	}
	...
	function renderChart(data) {
		...
		const compute = data.map((r) => r.compute_time);
		const smoothed = movingAvg(compute);
			...
			series: [
				{
					name: 'Compute (raw)',
					type: 'line',
					data: steps.map((s, i) => [s, compute[i]]),
	...
	$effect(() => {
		$stepTimeData;
		$dataWaitData;
		queueUpdate(buildComputeRows($stepTimeData, $dataWaitData));
	});
</script>

<div class="bg-gray-900 border border-gray-800 rounded-lg p-4">
	<h3 class="text-sm font-medium text-gray-400 mb-2">Compute Time</h3>
