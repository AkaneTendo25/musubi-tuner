<script>
	...
	let { label, value = $bindable(''), placeholder = '', tooltip = '', disabled = false, showFiles = false, onselect } = $props();
	...
	function selectCurrent() {
		value = browserPath;
		showBrowser = false;
		if (onselect) onselect(browserPath);
	}
...
<label class="block" data-tooltip={tooltip || undefined}>
	...
	<div class="mt-0.5 flex gap-1.5">
		<input
			...
			{placeholder}
			{disabled}
			class="flex-1 px-2.5 py-1.5 text-[13px] transition-colors disabled:opacity-40"
		...
		<button
			...
			onclick={openBrowser}
			{disabled}
			class="px-3 py-1.5 text-[13px] font-medium disabled:opacity-40 flex-shrink-0"
