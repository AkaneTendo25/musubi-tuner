...
export function estimateTraining(cfg) {
	...
	const ds = firstDataset(cfg);
	const isH3 = t.model_type === 'minimax_h3';

	const ltxVersion = String(t.ltx_version || '2.3');
	const ditBF16 = isH3 ? h3BaseSize({ ...t, fp8_base: false, fp8_scaled: false, h3_convrot_int8: false, int8_convrot_base: false }) : ltxVersion === '2.3' ? 42 : 39;
	const isFp8 = !!t.fp8_base;
	const isW8A8 = !!t.fp8_w8a8;
	const isNF4 = !!t.nf4_base;
	let ditBase = isH3 ? h3BaseSize(t) : isNF4 ? (ditBF16 / 4) : isFp8 ? (ditBF16 / 2) : ditBF16;

	const totalBlocks = isH3 ? 50 : 48;
	const configuredCheckpointBlocks = isH3 ? t.h3_gradient_checkpointing_blocks : blocksToCheckpointValue(t);
	...
	const usesCheckpointing = isH3 ? !!t.gradient_checkpointing : checkpointingActive(t);
	const blockwise = !isH3 && !!t.blockwise_checkpointing && blocksToCheckpoint !== 0;
	...
	const maxBlocksToSwap = isH3 && t.block_swap_h2d_only && t.block_swap_granularity === 'layer'
		? totalBlocks
		: isH3 ? totalBlocks - 2 : totalBlocks - 1;
	...
	const latentFrames = Math.max(1, Math.floor((sourceFrames - 1) / (isH3 ? 4 : 8)) + 1);
	...
	const hiddenDim = isH3 ? 5376 : mode === 'audio' ? 2048 : 4096;
