		...
		<div class="p-4 space-y-3" style="background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md);">
			...
			{#if caching.model_type === 'minimax_h3'}
				<div class="grid grid-cols-2 xl:grid-cols-4 gap-3">
					...
					<FormToggle label="One-frame images" fieldPath="caching.h3_one_frame" checked={caching.h3_one_frame ?? false} onchange={(e) => updateCaching('h3_one_frame', e.target.checked)} tooltip="Cache single-frame image targets. Timed FL2VA controls use the image dataset's control and target indices." />
			...
			{#if $advancedMode}
				<div class="grid grid-cols-2 xl:grid-cols-4 gap-3">
					<FormSelect fieldPath="caching.vae_dtype" value={caching.vae_dtype || ''} options={[{ value: '', label: caching.model_type === 'minimax_h3' ? 'float32 (default)' : 'bfloat16 (default)' }, 'float16', 'bfloat16', 'float32']} onchange={(e) => updateCaching('vae_dtype', e.target.value || null)} tooltip="Default stored latent dtype. H3 defaults to float32; its VAE retains checkpoint precision." />
					{#if caching.model_type === 'minimax_h3'}<FormSelect label="Latent cache dtype" fieldPath="caching.latent_cache_dtype" value={caching.latent_cache_dtype || ''} options={[{ value: '', label: 'Same as VAE (default)' }, 'float16', 'bfloat16', 'float32']} onchange={(e) => updateCaching('latent_cache_dtype', e.target.value || null)} tooltip="Storage dtype for H3 latent cache files. Changing it replaces mismatched caches when recached." />{/if}
