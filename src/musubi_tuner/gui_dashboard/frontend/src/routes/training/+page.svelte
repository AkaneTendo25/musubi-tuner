					...
					<div class="space-y-2 pt-2">
						{#if t.model_type === 'minimax_h3'}
							<div class="grid grid-cols-2 gap-x-4 gap-y-1">
								...
								<FormToggle label="Fused ConvRot LoRA" fieldPath="training.h3_convrot_int8_lora_fused" checked={t.h3_convrot_int8_lora_fused ?? false} onchange={(e) => update('h3_convrot_int8_lora_fused', e.target.checked)} disabled={!t.h3_convrot_int8 || t.h3_convrot_int8_fwd !== 'int8' || t.h3_convrot_int8_bwd !== 'int8'} tooltip="Fuse the LoRA-up projection into the ConvRot INT8 dequantization epilogue. Requires INT8 forward and backward." />
							...
							<div class="grid grid-cols-3 gap-2">
								<FormField label="AdaLN rank" type="number" fieldPath="training.h3_adaln_rank" value={t.h3_adaln_rank ?? ''} oninput={(e) => update('h3_adaln_rank', e.target.value ? Number(e.target.value) : null)} placeholder="Full rank" min={1} disabled={t.int8_convrot_base} tooltip="Factor the frozen AdaLN timestep projections to this rank while loading. Rank 16 is the usual compact setting for a BF16 base." invalid={fieldInvalid('training.h3_adaln_rank')} error={fieldError('training.h3_adaln_rank')} />
								...
								<FormSelect label="ConvRot forward" fieldPath="training.h3_convrot_int8_fwd" value={t.h3_convrot_int8_fwd || 'int8'} options={['int8', 'bf16']} onchange={(e) => update('h3_convrot_int8_fwd', e.target.value)} disabled={!t.h3_convrot_int8} tooltip="INT8 uses the fused quantized kernel; BF16 uses the vendor GEMM with the same stored ConvRot weights." />
								<FormSelect label="ConvRot backward" fieldPath="training.h3_convrot_int8_bwd" value={t.h3_convrot_int8_bwd || 'bf16'} options={['bf16', 'int8']} onchange={(e) => update('h3_convrot_int8_bwd', e.target.value)} disabled={!t.h3_convrot_int8 || t.h3_convrot_int8_fwd === 'bf16'} tooltip="BF16 is the more accurate default; INT8 is faster and enables the fused LoRA epilogue." />
