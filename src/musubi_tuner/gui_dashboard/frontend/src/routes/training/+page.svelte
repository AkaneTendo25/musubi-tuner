<script>
	...
	function updateQuantizedBaseMode(key, enabled) {
		if (enabled) {
			for (const modeKey of quantizedBaseModeKeys) {
				if (modeKey !== key) update(modeKey, false);
			}
			update('fp8_base', false);
			update('fp8_scaled', false);
			update('fp8_w8a8', false);
			update('nf4_base', false);
			update('loftq_init', false);
			update('h3_convrot_int8', false);
		}
		update(key, enabled);
	}
	...
	<div class="space-y-4">
		...
		<div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
			...
			<div class="space-y-3">
				<FormGroup title="Model">
					<div class="space-y-2 pt-2">
						...
						{#if t.model_type === 'minimax_h3'}
							...
							<div class="grid grid-cols-2 gap-2">
								<FormField type="number" fieldPath="training.h3_video_loss_weight" value={t.h3_video_loss_weight ?? 1} oninput={(e) => update('h3_video_loss_weight', Number(e.target.value || 1))} min={0} step="0.1" tooltip="Video loss weight" />
								<FormField type="number" fieldPath="training.h3_audio_loss_weight" value={t.h3_audio_loss_weight ?? 1} oninput={(e) => update('h3_audio_loss_weight', Number(e.target.value || 1))} min={0} step="0.1" tooltip="Audio loss weight" />
							</div>
							<FormField label="Spatial Density Jitter" type="number" fieldPath="training.h3_spatial_density_jitter" value={t.h3_spatial_density_jitter ?? 0} oninput={(e) => update('h3_spatial_density_jitter', Number(e.target.value))} min={0} step="0.05" tooltip="Perturb spatial RoPE density each step; 0 disables it. Enable only as an explicit regularizer." invalid={fieldInvalid('training.h3_spatial_density_jitter')} error={fieldError('training.h3_spatial_density_jitter')} />
							{#if $advancedMode}
								<div class="h3-control-cluster">
									...
									<FormField type="number" fieldPath="training.h3_timestep_focus_probability" value={t.h3_timestep_focus_probability ?? 0} oninput={(e) => update('h3_timestep_focus_probability', Number(e.target.value || 0))} min={0} max={1} step="0.05" tooltip="Optional share of video/AV timesteps sampled from the focus band. Zero preserves the native uniform schedule; image batches keep their resolution-aware schedule." />
									<FormField type="number" fieldPath="training.h3_timestep_focus_min" value={t.h3_timestep_focus_min ?? 0.4} oninput={(e) => update('h3_timestep_focus_min', Number(e.target.value))} min={0} max={1} step="0.05" tooltip="Lower edge of the unshifted video/AV base-sigma focus band." />
									<FormField type="number" fieldPath="training.h3_timestep_focus_max" value={t.h3_timestep_focus_max ?? 0.8} oninput={(e) => update('h3_timestep_focus_max', Number(e.target.value))} min={0} max={1} step="0.05" tooltip="Upper edge of the unshifted video/AV base-sigma focus band." />
										...
										<FormField type="number" fieldPath="training.h3_extension_probability" value={t.h3_extension_probability ?? 1} oninput={(e) => update('h3_extension_probability', Number(e.target.value || 1))} min={0} max={1} step="0.05" disabled={!(t.h3_extension_video_frames || t.h3_extension_audio_latents)} tooltip="Train the extension recipe on this synchronized fraction of steps; the rest train the plain objective. With masking also configured the two probabilities must not exceed 1 together." invalid={fieldInvalid('training.h3_extension_probability')} error={fieldError('training.h3_extension_probability')} />
								...
								<div class="h3-control-cluster">
									...
									<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
										...
										<FormField fieldPath="training.h3_guide_specs" value={t.h3_guide_specs || ''} oninput={(e) => update('h3_guide_specs', e.target.value)} placeholder="0:2:4;21:0:8" tooltip="Ref2VA pixel-start:video-latents:audio-latents guides. Visual starts use VAE boundaries; audio starts are multiples of 3 pixel frames." invalid={fieldInvalid('training.h3_guide_specs')} error={fieldError('training.h3_guide_specs')} />
										<FormSelect fieldPath="training.h3_mask_mode" value={t.h3_mask_mode || 'off'} options={['off', 'box', 'border', 'segment', 'dataset']} onchange={(e) => update('h3_mask_mode', e.target.value)} tooltip="Procedural H3 inpaint, outpaint, or temporal mask. Dataset instead reads the region authored by each dataset's conditioning mask directory." invalid={fieldInvalid('training.h3_mask_mode')} error={fieldError('training.h3_mask_mode')} />
									<FormToggle fieldPath="training.h3_mask_audio" checked={t.h3_mask_audio ?? false} onchange={(e) => update('h3_mask_audio', e.target.checked)} tooltip="Mask a contiguous audio span. This can be used alone or alongside a video mask." />
									<FormField type="number" fieldPath="training.h3_mask_min_fraction" value={t.h3_mask_min_fraction ?? 0.25} oninput={(e) => update('h3_mask_min_fraction', Number(e.target.value))} min={0} max={1} step="0.05" disabled={(t.h3_mask_mode === 'off' || t.h3_mask_mode === 'dataset') && !t.h3_mask_audio} />
									<FormField type="number" fieldPath="training.h3_mask_max_fraction" value={t.h3_mask_max_fraction ?? 0.75} oninput={(e) => update('h3_mask_max_fraction', Number(e.target.value))} min={0} max={1} step="0.05" disabled={(t.h3_mask_mode === 'off' || t.h3_mask_mode === 'dataset') && !t.h3_mask_audio} />
									<FormField type="number" fieldPath="training.h3_mask_probability" value={t.h3_mask_probability ?? 1} oninput={(e) => update('h3_mask_probability', Number(e.target.value || 1))} min={0} max={1} step="0.05" disabled={t.h3_mask_mode === 'off' && !t.h3_mask_audio} tooltip="Train the masked recipe on this synchronized fraction of steps; the rest train the plain objective. With extension also configured the two probabilities must not exceed 1 together." invalid={fieldInvalid('training.h3_mask_probability')} error={fieldError('training.h3_mask_probability')} />
									</div>
								</div>
								<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
									...
									<FormField fieldPath="training.h3_guidance_scale_range" value={t.h3_guidance_scale_range || ''} oninput={(e) => update('h3_guidance_scale_range', e.target.value)} placeholder="e.g. 2.5,3.5" disabled={t.h3_guidance_distillation_scale != null} tooltip="Draw the guidance scale uniformly in LOWER,UPPER per sample and step instead of pinning one value. Replaces the single distillation scale; 1 &lt; LOWER ≤ UPPER." invalid={fieldInvalid('training.h3_guidance_scale_range')} error={fieldError('training.h3_guidance_scale_range')} />
										<FormField type="number" fieldPath="training.h3_guidance_distillation_probability" value={t.h3_guidance_distillation_probability ?? 1} oninput={(e) => update('h3_guidance_distillation_probability', Number(e.target.value || 1))} min={0.01} max={1} step="0.05" disabled={!(t.h3_guidance_distillation_scale > 1) && !t.h3_guidance_scale_range} tooltip="Run the guidance empty forward on this fraction of steps. Active losses are scaled automatically by 1/probability; 0.25–0.5 is the recommended faster range." invalid={fieldInvalid('training.h3_guidance_distillation_probability')} error={fieldError('training.h3_guidance_distillation_probability')} />
									...
									<FormSelect fieldPath="training.h3_guidance_loss_schedule" value={t.h3_guidance_loss_schedule || 'sigma'} options={['sigma', 'constant']} onchange={(e) => update('h3_guidance_loss_schedule', e.target.value)} tooltip="Sigma decays guidance toward 1 at the clean endpoint; constant uses the configured scale everywhere" />
									<FormSelect fieldPath="training.h3_guidance_null_source" value={t.h3_guidance_null_source || 'live'} options={['live', 'frozen']} onchange={(e) => update('h3_guidance_null_source', e.target.value)} disabled={!(t.h3_guidance_distillation_scale > 1) && !t.h3_guidance_scale_range} tooltip="Live evaluates the null branch with the trainable adapter active; frozen disables the adapter for that forward so the guidance correction inverts a fixed base field" invalid={fieldInvalid('training.h3_guidance_null_source')} error={fieldError('training.h3_guidance_null_source')} />
									<FormToggle fieldPath="training.h3_guidance_cfg_zero" checked={t.h3_guidance_cfg_zero ?? false} onchange={(e) => update('h3_guidance_cfg_zero', e.target.checked)} disabled={!(t.h3_guidance_distillation_scale > 1) && !t.h3_guidance_scale_range} tooltip="CFG-Zero* rescale: project the null branch onto the conditional field per sample and modality before the guidance form is applied" />
									<FormField type="number" fieldPath="training.h3_base_preservation_loss_weight" value={t.h3_base_preservation_loss_weight ?? 0} oninput={(e) => update('h3_base_preservation_loss_weight', Number(e.target.value || 0))} min={0} step="0.01" tooltip="Penalize drift from the frozen base prediction; 0.02 is the recommended starting value and adds one no-grad transformer forward on active batches" invalid={fieldInvalid('training.h3_base_preservation_loss_weight')} error={fieldError('training.h3_base_preservation_loss_weight')} />
									<FormField type="number" fieldPath="training.h3_base_preservation_probability" value={t.h3_base_preservation_probability ?? 1} oninput={(e) => update('h3_base_preservation_probability', Number(e.target.value || 1))} min={0.01} max={1} step="0.05" disabled={!(t.h3_base_preservation_loss_weight > 0)} tooltip="Run the frozen-base forward on this fraction of batches. Active losses are scaled automatically by 1/probability; 0.25–0.5 is the recommended faster range." invalid={fieldInvalid('training.h3_base_preservation_probability')} error={fieldError('training.h3_base_preservation_probability')} />
									<FormToggle label="Fuse Frozen Teachers" fieldPath="training.h3_fuse_frozen_teachers" checked={t.h3_fuse_frozen_teachers ?? false} onchange={(e) => update('h3_fuse_frozen_teachers', e.target.checked)} disabled={t.h3_guidance_null_source !== 'frozen' || !(t.h3_base_preservation_loss_weight > 0) || (!(t.h3_guidance_distillation_scale > 1) && !t.h3_guidance_scale_range)} tooltip="Batch the frozen empty-guidance and preservation teachers into one forward. This reduces auxiliary overhead but can raise transient teacher memory." invalid={fieldInvalid('training.h3_fuse_frozen_teachers')} error={fieldError('training.h3_fuse_frozen_teachers')} />
								</div>
								<PathInput fieldPath="training.h3_overlay_weights" value={t.h3_overlay_weights || ''} oninput={(e) => update('h3_overlay_weights', e.target.value)} showFiles tooltip="Optional frozen LoRA kept live on top of the base by default: never merged, never trained, never saved into the trained adapter. Works with an INT8 ConvRot base, where merging would have to requantize." />
								<div class="grid grid-cols-2 gap-2">
									<FormField type="number" fieldPath="training.h3_overlay_weights_multiplier" value={t.h3_overlay_weights_multiplier ?? 1} oninput={(e) => update('h3_overlay_weights_multiplier', Number(e.target.value || 1))} step="0.05" disabled={!t.h3_overlay_weights} tooltip="Strength of the live overlay LoRA (default 1.0)." invalid={fieldInvalid('training.h3_overlay_weights_multiplier')} error={fieldError('training.h3_overlay_weights_multiplier')} />
									<FormToggle label="Training Only" fieldPath="training.h3_overlay_training_only" checked={t.h3_overlay_training_only ?? false} onchange={(e) => update('h3_overlay_training_only', e.target.checked)} disabled={!t.h3_overlay_weights} tooltip="Keep the frozen overlay active for training and, unless Validate Without Overlay is enabled, data-loss validation, but turn it off for previews. The overlay is always excluded from the saved LoRA." invalid={fieldInvalid('training.h3_overlay_training_only')} error={fieldError('training.h3_overlay_training_only')} />
									<FormToggle label="Validate Without Overlay" fieldPath="training.h3_validate_without_overlay" checked={t.h3_validate_without_overlay ?? false} onchange={(e) => update('h3_validate_without_overlay', e.target.checked)} disabled={!t.h3_overlay_training_only} tooltip="Disable the overlay for the whole validation pass, data loss included, so validation metrics and the field and rollout probes describe the learned LoRA on the original checkpoint." invalid={fieldInvalid('training.h3_validate_without_overlay')} error={fieldError('training.h3_validate_without_overlay')} />
								</div>
								<PathInput fieldPath="training.dataset_manifest" value={t.dataset_manifest || ''} oninput={(e) => update('dataset_manifest', e.target.value)} showFiles tooltip="Optional dataset manifest" />
								<PathInput fieldPath="training.config_file" value={t.config_file || ''} oninput={(e) => update('config_file', e.target.value)} showFiles tooltip="Optional trainer TOML config" />
							{/if}
							<div class="grid grid-cols-2 gap-2">
								<FormToggle fieldPath="training.fp8_base" checked={(t.fp8_base ?? false) || (t.fp8_scaled ?? false)} onchange={(e) => { if (e.target.checked) { update('int8_convrot_base', false); update('h3_convrot_int8', false); } update('fp8_base', e.target.checked); update('fp8_scaled', false); }} tooltip="Quantize the full H3 checkpoint to scaled FP8 while loading" />
								<FormToggle label="Pruned INT8 ConvRot Base" fieldPath="training.int8_convrot_base" checked={t.int8_convrot_base ?? false} onchange={(e) => updateQuantizedBaseMode('int8_convrot_base', e.target.checked)} tooltip="Load the pruned Comfy INT8 ConvRot checkpoint and train LoRA adapters with W8A8 compute" />
							</div>
						{:else}
				...
				{#if $advancedMode && t.model_type !== 'minimax_h3'}
					<FormGroup title="Quantization">
						<div class="space-y-2 pt-2">
							...
							<div class="grid grid-cols-3 gap-x-4 gap-y-1">
								<FormToggle fieldPath="training.int8_base" checked={t.int8_base ?? false} onchange={(e) => updateQuantizedBaseMode('int8_base', e.target.checked)} tooltip="Train a LoRA over a pre-quantized Optimum-Quanto int8 checkpoint (needs Triton). Mutually exclusive with FP8/NF4." />
								<FormToggle fieldPath="training.int8_base_dynamic" checked={t.int8_base_dynamic ?? false} onchange={(e) => updateQuantizedBaseMode('int8_base_dynamic', e.target.checked)} tooltip="Quantize a standard (bf16) checkpoint to int8 on the fly at load time (no pre-quantized file needed)." />
								<FormToggle label="Pruned INT8 ConvRot Base" fieldPath="training.int8_convrot_base" checked={t.int8_convrot_base ?? false} onchange={(e) => updateQuantizedBaseMode('int8_convrot_base', e.target.checked)} tooltip="Load a pre-quantized INT8 ConvRot checkpoint." />
								<FormToggle label="INT8 ConvRot Dynamic" fieldPath="training.int8_convrot_dynamic" checked={t.int8_convrot_dynamic ?? false} onchange={(e) => updateQuantizedBaseMode('int8_convrot_dynamic', e.target.checked)} tooltip="Quantize a standard checkpoint to INT8 ConvRot at load time." />
								<FormToggle fieldPath="training.int8_fused_quant" checked={t.int8_fused_quant ?? false} onchange={(e) => update('int8_fused_quant', e.target.checked)} tooltip="Request fused Triton activation/gradient quantization for eligible INT8 Linear calls; requires Triton and CUDA." />
								<FormToggle label="INT8 ConvRot No MSE Clip" fieldPath="training.int8_convrot_no_mse_clip" checked={t.int8_convrot_no_mse_clip ?? false} onchange={(e) => update('int8_convrot_no_mse_clip', e.target.checked)} disabled={!(t.int8_convrot_base || t.int8_convrot_dynamic)} tooltip="Use absmax instead of the MSE-selected clipping scale." />
							</div>
							<div class="grid grid-cols-2 gap-2">
							...
							<div class="grid grid-cols-3 gap-x-4 gap-y-1">
								<FormToggle label="W4A4G4" fieldPath="training.w4a4g4" checked={t.w4a4g4 ?? false} onchange={(e) => updateQuantizedBaseMode('w4a4g4', e.target.checked)} tooltip="Fully 4-bit (W4A4G4) LoRA training over a frozen 4-bit base; the container is selected below. Mutually exclusive with W4A8, W4A4G8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="W4A8" fieldPath="training.w4a8" checked={t.w4a8 ?? false} onchange={(e) => updateQuantizedBaseMode('w4a8', e.target.checked)} tooltip="4-bit weights with 8-bit activations (int4 container only). Mutually exclusive with W4A4G4, W4A4G8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="W4A4G8" fieldPath="training.w4a4g8" checked={t.w4a4g8 ?? false} onchange={(e) => updateQuantizedBaseMode('w4a4g8', e.target.checked)} tooltip="4-bit weights/activations with row-wise INT8 grad-output; int4 container only. Mutually exclusive with W4A4G4, W4A8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="INT4 No MSE Clip" fieldPath="training.int4_convrot_no_mse_clip" checked={t.int4_convrot_no_mse_clip ?? false} onchange={(e) => update('int4_convrot_no_mse_clip', e.target.checked)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8 || t.int4_convrot_base || t.int4_convrot_dynamic)} tooltip="Use plain absmax row scales instead of the MSE-selected clipping scale during on-the-fly INT4 ConvRot quantization." />
							</div>
							<div class="grid grid-cols-2 gap-x-4 gap-y-1">
								<FormToggle label="INT4 ConvRot Base" fieldPath="training.int4_convrot_base" checked={t.int4_convrot_base ?? false} onchange={(e) => updateQuantizedBaseMode('int4_convrot_base', e.target.checked)} tooltip="Train a LoRA over a pre-quantized packed INT4 ConvRot checkpoint. Mutually exclusive with FP8/NF4/INT8 base modes." />
								<FormToggle label="INT4 ConvRot Dynamic" fieldPath="training.int4_convrot_dynamic" checked={t.int4_convrot_dynamic ?? false} onchange={(e) => updateQuantizedBaseMode('int4_convrot_dynamic', e.target.checked)} tooltip="Quantize a standard checkpoint to packed INT4 ConvRot at load time." />
							</div>
							<div class="grid grid-cols-3 gap-2">
							...
							<div class="grid grid-cols-3 gap-2">
								<FormField label="INT4 Groupsize" fieldPath="training.int4_convrot_groupsize" value={t.int4_convrot_groupsize || 'auto'} oninput={(e) => update('int4_convrot_groupsize', e.target.value || 'auto')} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8 || t.int4_convrot_base || t.int4_convrot_dynamic)} placeholder="auto" tooltip="INT4 ConvRot group size or comma list. Auto tries 256,64,16." />
								<FormField label="INT4 Quality Report" fieldPath="training.int4_convrot_quality_report" value={t.int4_convrot_quality_report || ''} oninput={(e) => update('int4_convrot_quality_report', e.target.value)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8 || t.int4_convrot_base || t.int4_convrot_dynamic)} placeholder="Optional JSON path" tooltip="Write per-layer reconstruction metrics during on-the-fly INT4 ConvRot quantization." />
								<FormSelect label="W4A4G4 Container" fieldPath="training.w4a4g4_container" value={t.w4a4g4_container || 'auto'} options={['auto', 'int4', 'nvfp4']} onchange={(e) => update('w4a4g4_container', e.target.value)} disabled={!t.w4a4g4} tooltip="Frozen-base numeric format for W4A4G4. auto detects from the checkpoint (int4cr→int4, NVFP4→nvfp4, bf16→int4). int4 = INT4 tensor cores + ConvRot rotation; nvfp4 = no-rotation format (Blackwell)." />
								<FormField type="number" label="W4A4G4 Stabilizer Rank" fieldPath="training.w4a4g4_stabilizer_rank" value={t.w4a4g4_stabilizer_rank ?? 0} oninput={(e) => update('w4a4g4_stabilizer_rank', Number(e.target.value))} min={0} disabled={!(t.w4a4g4 || t.w4a4g8)} tooltip="Frozen low-rank stabilizer rank for on-the-fly quantization of a bf16 checkpoint. 0 = container default (int4→0, nvfp4→32). Applies to W4A4G4 and W4A4G8; ignored for pre-quantized checkpoints and W4A8." />
							</div>
							<div class="grid grid-cols-3 gap-2">
								<FormField type="number" label="INT4 Scale Refine Steps" fieldPath="training.int4_convrot_scale_refine_steps" value={t.int4_convrot_scale_refine_steps ?? 0} oninput={(e) => update('int4_convrot_scale_refine_steps', Number(e.target.value))} min={0} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} tooltip="Optional least-squares refinement after INT4 clipping. 0 preserves the existing quantizer; 2 enables the calibrated path." />
								<FormField type="number" label="INT4 Group Scales" fieldPath="training.int4_convrot_group_scales" value={t.int4_convrot_group_scales ?? 0} oninput={(e) => update('int4_convrot_group_scales', Number(e.target.value))} min={0} step={16} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} tooltip="Maximum K-group size for per-group INT4 weight scales. 0 disables; enter an explicit power of two such as 128 or 64." />
								<FormToggle label="INT4 Q8.8 Group Ratios" fieldPath="training.int4_convrot_group_ratio_q8" checked={t.int4_convrot_group_ratio_q8 ?? false} onchange={(e) => update('int4_convrot_group_ratio_q8', e.target.checked)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8) || !(t.int4_convrot_group_scales > 0)} tooltip="Halve group-ratio checkpoint/VRAM storage with an int16 Q8.8 representative that preserves every deployed signed-INT4 mapping." />
								<FormField label="Compare INT4 Group Scales" fieldPath="training.int4_convrot_compare_group_scales" value={t.int4_convrot_compare_group_scales || ''} oninput={(e) => update('int4_convrot_compare_group_scales', e.target.value)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8) || !t.int4_convrot_quality_report} placeholder="e.g. 0,128,64" tooltip="Explicit report-only candidate list. Every value is measured; none is selected or applied automatically." />
								<FormField label="ConvRot Policy" fieldPath="training.convrot_policy" value={t.convrot_policy || ''} oninput={(e) => update('convrot_policy', e.target.value)} disabled={!(t.int8_convrot_base || t.int8_convrot_dynamic || t.w4a4g4 || t.w4a8 || t.w4a4g8)} placeholder="Optional policy JSON path" tooltip="Manual ordered per-layer rules for quantize, compute, group_scales, group_ratio_q8, and scale_refine_steps." />
							</div>
							<div class="grid grid-cols-3 gap-2">
								<FormSelect fieldPath="training.w8a8_backend" value={t.w8a8_backend || ''} options={[{value:'',label:'Default'},{value:'torch',label:'torch'},{value:'triton',label:'triton'},{value:'cutlass',label:'cutlass'}]} onchange={(e) => update('w8a8_backend', e.target.value || null)} disabled={(!t.fp8_w8a8 || (t.w8a8_mode || 'int8') !== 'int8') && !t.int8_base && !t.int8_base_dynamic && !t.int8_convrot_base && !t.int8_convrot_dynamic} tooltip="Manual int8 matmul backend selector; leave blank to preserve existing behavior." />
				...
				<FormGroup title="Schedule">
					<div class="space-y-2 pt-2">
						...
						<div class="grid grid-cols-2 gap-2">
							{#if t.model_type === 'minimax_h3'}
								<FormSelect fieldPath="training.h3_timestep_sampling" value={t.h3_timestep_sampling || 'uniform'} options={['uniform', 'sigmoid', 'shift', 'logsnr', 'sigma']} onchange={(e) => update('h3_timestep_sampling', e.target.value)} tooltip="Shape of H3's shared unshifted base schedule. Uniform matches the released video/audio training contract; H3 applies shifts 12 and 3 afterward." />
							{:else}
					...
					<div class="space-y-2 pt-2">
						...
						{#if $advancedMode}
							...
							<div class="grid grid-cols-3 gap-x-4 gap-y-1">
								...
								<FormToggle label="H2D-only block swap" fieldPath="training.block_swap_h2d_only" checked={t.block_swap_h2d_only ?? false} onchange={(e) => update('block_swap_h2d_only', e.target.checked)} disabled={!t.blocks_to_swap || t.ltx2_model_parallel || t.ltx2_remote_stage} tooltip="Frozen-base LoRA block swap: keep CPU masters and stream host-to-device only. Requires gradient checkpointing." />
							...
							<FormField label="Block swap ring size" type="number" fieldPath="training.block_swap_ring_size" value={t.block_swap_ring_size ?? 2} oninput={(e) => update('block_swap_ring_size', Number(e.target.value || 2))} min={1} disabled={!t.block_swap_h2d_only} tooltip="GPU ring buffers for H2D-only block swap. 2 is double buffering; 1 uses less VRAM with less overlap." />
				...
				<FormGroup title={t.model_type === 'minimax_h3' ? 'H3 Performance' : 'LTX-2 Performance'}>
					<div class="space-y-2 pt-2">
						{#if t.model_type === 'minimax_h3'}
							<div class="grid grid-cols-2 gap-x-4 gap-y-1">
								<FormSelect label="Fused QK norm + RoPE" fieldPath="training.h3_fused_qk_norm_rope" value={t.h3_fused_qk_norm_rope == null ? 'auto' : String(t.h3_fused_qk_norm_rope)} options={['auto', { value: 'true', label: 'On' }, { value: 'false', label: 'Off' }]} onchange={(e) => update('h3_fused_qk_norm_rope', e.target.value === 'auto' ? null : e.target.value === 'true')} tooltip="Auto uses the Triton kernel when available; compiled or unsupported cases fall back. Off emits the explicit opt-out." />
								<FormSelect label="Fused indexed AdaLN" fieldPath="training.h3_fused_indexed_adaln" value={t.h3_fused_indexed_adaln == null ? 'auto' : String(t.h3_fused_indexed_adaln)} options={['auto', { value: 'true', label: 'On' }, { value: 'false', label: 'Off' }]} onchange={(e) => update('h3_fused_indexed_adaln', e.target.value === 'auto' ? null : e.target.value === 'true')} tooltip="Auto fuses eligible frozen-base eager CUDA calls; unsupported cases fall back. Off emits the explicit opt-out." />
								...
								<FormSelect label="Fused SwiGLU" fieldPath="training.h3_fused_swiglu" value={t.h3_fused_swiglu == null ? 'auto' : String(t.h3_fused_swiglu)} options={['auto', { value: 'true', label: 'On' }, { value: 'false', label: 'Off' }]} onchange={(e) => update('h3_fused_swiglu', e.target.value === 'auto' ? null : e.target.value === 'true')} tooltip="Auto uses the Triton kernel when available; compiled or unsupported cases fall back. Off emits the explicit opt-out." />
								...
								<FormToggle label="Attention autotune" fieldPath="training.h3_attn_autotune" checked={t.h3_attn_autotune ?? false} onchange={(e) => update('h3_attn_autotune', e.target.checked)} disabled={!t.sdpa || (t.h3_attn_auto_dispatch ?? false)} tooltip="Probe every runnable unmasked attention backend (SDPA, cuDNN-priority SDPA, installed FlashAttention builds) on the real packed shape once per sequence bucket and use the measured winner. Requires SDPA; replaces auto-dispatch." />
								...
								<FormToggle label="Train text token refiner" fieldPath="training.h3_lora_token_refiner" checked={t.h3_lora_token_refiner ?? false} onchange={(e) => update('h3_lora_token_refiner', e.target.checked)} tooltip="Also place LoRA adapters on H3's two text token-refiner blocks. Experimental option for stronger trigger and identity binding; existing behavior is unchanged when off." />
								...
								<FormToggle label="Fused BF16 LoRA" fieldPath="training.h3_lora_fused_bf16" checked={t.h3_lora_fused_bf16 ?? false} onchange={(e) => update('h3_lora_fused_bf16', e.target.checked)} disabled={t.h3_training_type === 'learned_context' || t.h3_convrot_int8 || t.int8_convrot_base || t.fp8_base || t.fp8_scaled} tooltip="Fold the LoRA delta add into eligible bias-free BF16 Linear GEMMs. Unsupported modules and compiled regions fall back to the regular path." />
								<FormToggle label="Varlen padding" fieldPath="training.h3_varlen_padding" checked={t.h3_varlen_padding ?? false} onchange={(e) => update('h3_varlen_padding', e.target.checked)} tooltip="Use FlashAttention variable-length packing for key-validity padding masks. Falls back to masked SDPA when unavailable or for pairwise masks." />
								<FormToggle label="Keep DiT resident for samples" fieldPath="training.h3_sample_keep_dit_resident" checked={t.h3_sample_keep_dit_resident ?? false} onchange={(e) => update('h3_sample_keep_dit_resident', e.target.checked)} tooltip="Keep H3 weights on the GPU while sample decoders run. Requires enough VRAM for decoding; an OOM automatically retries after evacuation." />
							...
							<div class="grid grid-cols-3 gap-2">
								...
								<FormSelect label="FP8 scale granularity" fieldPath="training.h3_fp8_quantization_mode" value={t.h3_fp8_quantization_mode || 'block'} options={['block', 'channel', 'tensor']} onchange={(e) => update('h3_fp8_quantization_mode', e.target.value)} disabled={!(t.fp8_base || t.fp8_scaled)} tooltip="Scale granularity for H3 FP8 weights. Block is the finest-grained default." />
								<FormField label="Checkpointed H3 blocks" type="number" fieldPath="training.h3_gradient_checkpointing_blocks" value={t.h3_gradient_checkpointing_blocks ?? ''} oninput={(e) => update('h3_gradient_checkpointing_blocks', e.target.value === '' ? null : Number(e.target.value))} placeholder="All 50" min={0} max={50} disabled={!t.gradient_checkpointing} tooltip="Checkpoint only the last N of H3's 50 main blocks. Values below 50 require resident eager blocks and cannot use block swap or torch.compile." invalid={fieldInvalid('training.h3_gradient_checkpointing_blocks')} error={fieldError('training.h3_gradient_checkpointing_blocks')} />
								<FormToggle label="Checkpoint swapped blocks" fieldPath="training.h3_gradient_checkpointing_swapped" checked={t.h3_gradient_checkpointing_swapped ?? false} onchange={(e) => update('h3_gradient_checkpointing_swapped', e.target.checked)} disabled={!t.gradient_checkpointing || !(t.blocks_to_swap > 0) || t.h3_gradient_checkpointing_blocks != null} tooltip="Checkpoint exactly the streamed blocks and leave resident blocks eager. Requires gradient checkpointing and block swap; uses more VRAM to reduce recomputation." invalid={fieldInvalid('training.h3_gradient_checkpointing_swapped')} error={fieldError('training.h3_gradient_checkpointing_swapped')} />
								<FormSelect label="ConvRot forward" fieldPath="training.h3_convrot_int8_fwd" value={t.h3_convrot_int8_fwd || 'int8'} options={['int8', 'bf16']} onchange={(e) => update('h3_convrot_int8_fwd', e.target.value)} disabled={!(t.h3_convrot_int8 || t.int8_convrot_base)} tooltip="INT8 uses the fused quantized kernel; BF16 uses the vendor GEMM with the same stored online or pre-quantized ConvRot weights." />
								<FormSelect label="ConvRot backward" fieldPath="training.h3_convrot_int8_bwd" value={t.h3_convrot_int8_bwd || 'bf16'} options={['bf16', 'int8']} onchange={(e) => update('h3_convrot_int8_bwd', e.target.value)} disabled={!(t.h3_convrot_int8 || t.int8_convrot_base) || t.h3_convrot_int8_fwd === 'bf16'} tooltip="BF16 is the more accurate default; INT8 is faster and enables the fused LoRA epilogue for online and pre-quantized ConvRot weights." />
								<FormSelect label="Checkpoint offload dtype" fieldPath="training.gradient_checkpointing_cpu_offload_dtype" value={t.gradient_checkpointing_cpu_offload_dtype || 'none'} options={[{value:'none',label:'Native precision'},{value:'fp8_e4m3',label:'FP8 E4M3'}]} onchange={(e) => update('gradient_checkpointing_cpu_offload_dtype', e.target.value)} disabled={!t.gradient_checkpointing || !t.gradient_checkpointing_cpu_offload || !t.h3_reusable_activation_offload} tooltip="Wire dtype for CPU-offloaded checkpoint activations. FP8 E4M3 halves PCIe traffic and pinned host memory for BF16/FP16 activations at the cost of a slightly lossy recomputation. Requires reusable activation offload." invalid={fieldInvalid('training.gradient_checkpointing_cpu_offload_dtype')} error={fieldError('training.gradient_checkpointing_cpu_offload_dtype')} />
								<FormSelect label="Block swap granularity" fieldPath="training.block_swap_granularity" value={t.block_swap_granularity || 'block'} options={['block', 'layer']} onchange={(e) => update('block_swap_granularity', e.target.value)} disabled={!t.block_swap_h2d_only} tooltip="Stream packed transformer blocks for speed or individual frozen layers for lower VRAM." />
							</div>
						{:else}
						<div class="grid grid-cols-2 gap-x-4 gap-y-1">
							<FormToggle label="Fused QK norm + RoPE" fieldPath="training.ltx2_fused_norm_rope" checked={t.ltx2_fused_norm_rope ?? false} onchange={(e) => update('ltx2_fused_norm_rope', e.target.checked)} tooltip="Fuse Q/K RMSNorm and RoPE; typically saves about 6% step time on eligible BF16 CUDA shapes." />
							<FormToggle label="Fused norm/RoPE backward" fieldPath="training.ltx2_fused_norm_rope_backward" checked={t.ltx2_fused_norm_rope_backward ?? false} onchange={(e) => update('ltx2_fused_norm_rope_backward', e.target.checked)} disabled={!t.ltx2_fused_norm_rope} tooltip="Fuse the matching backward; usually adds another 1.5-3%." />
							<FormToggle label="Compact AV cross-AdaLN" fieldPath="training.ltx2_compact_av_cross_adaln" checked={t.ltx2_compact_av_cross_adaln ?? false} onchange={(e) => update('ltx2_compact_av_cross_adaln', e.target.checked)} disabled={t.ltx2_mode !== 'av'} tooltip="Reuse repeated AV cross-AdaLN timestep projections; useful when many tokens share conditioning timesteps." />
							<FormToggle label="FP8 placement scope" fieldPath="training.ltx2_fp8_placement_scope" checked={t.ltx2_fp8_placement_scope ?? false} onchange={(e) => update('ltx2_fp8_placement_scope', e.target.checked)} disabled={!(t.fp8_base && t.fp8_scaled)} tooltip="Avoid repeated scaled-FP8 placement checks during a forward; can save about 8% on short checkpointed workloads." />
							<FormToggle label="Attention auto-dispatch" fieldPath="training.ltx2_attn_auto_dispatch" checked={t.ltx2_attn_auto_dispatch ?? false} onchange={(e) => update('ltx2_attn_auto_dispatch', e.target.checked)} disabled={!t.sdpa} tooltip="Use cuDNN-prioritized SDPA for large maskless shapes; most useful for long sequences." />
							<FormToggle label="FlashAttention 4" fieldPath="training.ltx2_flash_attn_4" checked={t.ltx2_flash_attn_4 ?? false} onchange={(e) => update('ltx2_flash_attn_4', e.target.checked)} disabled={!t.sdpa || !(t.ltx2_attn_auto_dispatch ?? false)} tooltip="Use optional FlashAttention-4 for eligible large head-dim-128 shapes. Requires attention auto-dispatch and a compatible flash-attn-4 installation." />
							<FormToggle label="Preserve prompt K/V" fieldPath="training.ltx2_prompt_kv_checkpoint" checked={t.ltx2_prompt_kv_checkpoint ?? false} onchange={(e) => update('ltx2_prompt_kv_checkpoint', e.target.checked)} disabled={!t.gradient_checkpointing} tooltip="Avoid rebuilding video prompt K/V during checkpoint recomputation; useful for long prompts." />
							<FormToggle label="Trim padded prompt" fieldPath="training.ltx2_padded_prompt_trim" checked={t.ltx2_padded_prompt_trim ?? false} onchange={(e) => update('ltx2_padded_prompt_trim', e.target.checked)} tooltip="Remove a shared right-padded prompt tail before transfer and attention." />
							<FormToggle label="Partial gradient checkpointing" fieldPath="training.ltx2_partial_gradient_checkpointing" checked={t.ltx2_partial_gradient_checkpointing ?? false} onchange={(e) => update('ltx2_partial_gradient_checkpointing', e.target.checked)} disabled={!t.gradient_checkpointing} tooltip="Retain early-block activations for speed at higher VRAM cost; keep the checkpoint count near 48 for long sequences." />
							<FormToggle label="Bounded activation offload" fieldPath="training.ltx2_bounded_activation_offload" checked={t.ltx2_bounded_activation_offload ?? false} onchange={(e) => update('ltx2_bounded_activation_offload', e.target.checked)} disabled={!t.gradient_checkpointing} tooltip="Asynchronously offload only LTX checkpoint-boundary activations with bounded GPU run-ahead and backward prefetch. Experimental and default-off." />
							<FormToggle label="Compile inner LTX blocks" fieldPath="training.ltx2_compile_inner_blocks" checked={t.ltx2_compile_inner_blocks ?? false} onchange={(e) => update('ltx2_compile_inner_blocks', e.target.checked)} disabled={!t.compile || (t.blocks_to_swap ?? 0) > 0 || t.blockwise_checkpointing || t.gradient_checkpointing_cpu_offload} tooltip="Compile block compute while keeping checkpoint wrappers eager; measured faster for resident-weight LoRA." />
							<FormToggle label="Async backward block prefetch" fieldPath="training.ltx2_block_swap_async_backward" checked={t.ltx2_block_swap_async_backward ?? false} onchange={(e) => update('ltx2_block_swap_async_backward', e.target.checked)} disabled={!t.gradient_checkpointing || !t.use_pinned_memory_for_block_swap || !(t.blocks_to_swap > 0)} tooltip="Overlap the next pinned block load with backward compute; measured about 8% faster for 512×512×33 full fine-tuning." />
							<FormToggle label="Validate training tensors" fieldPath="training.ltx2_validate_training_tensors" checked={t.ltx2_validate_training_tensors ?? false} onchange={(e) => update('ltx2_validate_training_tensors', e.target.checked)} tooltip="Scan cached inputs, predictions, and targets for NaN/Inf every step. Enable only when diagnosing instability." />
							<FormField label="XM noise candidates" type="number" fieldPath="training.ltx2_xm_k" value={t.ltx2_xm_k ?? 1} oninput={(e) => update('ltx2_xm_k', Number(e.target.value))} min={1} max={32} step={1} tooltip="Forward Explorative Modeling candidates per sample. 1 disables XM; larger values add no-gradient scoring forwards before the selected training forward." />
						</div>
						<div class="grid grid-cols-3 gap-2">
							<FormField label="Compact min tokens" type="number" fieldPath="training.ltx2_compact_av_cross_adaln_min_tokens" value={t.ltx2_compact_av_cross_adaln_min_tokens ?? ''} oninput={(e) => update('ltx2_compact_av_cross_adaln_min_tokens', e.target.value ? Number(e.target.value) : null)} placeholder="256" min={1} disabled={!t.ltx2_compact_av_cross_adaln} />
							<FormField label="Prompt K/V limit (MiB)" type="number" fieldPath="training.ltx2_prompt_kv_checkpoint_max_mb" value={t.ltx2_prompt_kv_checkpoint_max_mb ?? ''} oninput={(e) => update('ltx2_prompt_kv_checkpoint_max_mb', e.target.value ? Number(e.target.value) : null)} placeholder="1024" min={0} disabled={!t.ltx2_prompt_kv_checkpoint} />
							<FormField label="Blocks to checkpoint" type="number" fieldPath="training.blocks_to_checkpoint" value={t.blocks_to_checkpoint ?? ''} oninput={(e) => update('blocks_to_checkpoint', e.target.value ? Number(e.target.value) : null)} placeholder="All" disabled={!t.blockwise_checkpointing && !t.ltx2_partial_gradient_checkpointing} tooltip="For partial checkpointing, use a lower count for short sequences and a count near 48 for long sequences." />
							<FormField label="Offload in-flight limit" type="number" fieldPath="training.ltx2_activation_offload_max_inflight" value={t.ltx2_activation_offload_max_inflight ?? ''} oninput={(e) => update('ltx2_activation_offload_max_inflight', e.target.value ? Number(e.target.value) : null)} placeholder="2" min={1} disabled={!t.ltx2_bounded_activation_offload} />
							<FormField label="Resident trailing blocks" type="number" fieldPath="training.ltx2_activation_offload_keep_trailing" value={t.ltx2_activation_offload_keep_trailing ?? ''} oninput={(e) => update('ltx2_activation_offload_keep_trailing', e.target.value ? Number(e.target.value) : null)} placeholder="2" min={0} disabled={!t.ltx2_bounded_activation_offload} />
							<FormField label="Offload minimum (MiB)" type="number" fieldPath="training.ltx2_activation_offload_min_mb" value={t.ltx2_activation_offload_min_mb ?? ''} oninput={(e) => update('ltx2_activation_offload_min_mb', e.target.value ? Number(e.target.value) : null)} placeholder="1" min={0} step="0.25" disabled={!t.ltx2_bounded_activation_offload} />
						</div>
						{/if}
					</div>
				</FormGroup>
				...
				<div class="space-y-3 pt-2">
					...
					<FormGroup title="Guidance and measured variance" collapsed={true}>
						<div class="grid grid-cols-2 xl:grid-cols-4 gap-2 pt-2">
							...
							<FormToggle label="Reuse empty null anchor" fieldPath="training.h3_null_anchor_reuse_empty" checked={t.h3_null_anchor_reuse_empty ?? false} onchange={(e) => update('h3_null_anchor_reuse_empty', e.target.checked)} disabled={!t.h3_guidance_distillation_scale || t.h3_guidance_null_source !== 'frozen' || !(t.h3_guidance_null_anchor_weight > 0)} tooltip="Reuse the guidance step's frozen empty-prompt prediction as the null-anchor reference, saving one transformer pass." invalid={fieldInvalid('training.h3_null_anchor_reuse_empty')} error={fieldError('training.h3_null_anchor_reuse_empty')} />
							<FormToggle label="Shared null-anchor draws" fieldPath="training.h3_null_anchor_shared_draws" checked={t.h3_null_anchor_shared_draws ?? false} onchange={(e) => update('h3_null_anchor_shared_draws', e.target.checked)} disabled={!(t.h3_guidance_null_anchor_weight > 0) || t.h3_null_anchor_reuse_empty} tooltip="Replay the student's entry RNG for the frozen null-anchor reference so both sides use the same conditioning-noise draw." invalid={fieldInvalid('training.h3_null_anchor_shared_draws')} error={fieldError('training.h3_null_anchor_shared_draws')} />
		...
		{#if $advancedMode}
					...
					<FormGroup title="Remote Master">
						<div class="space-y-3 pt-2">
							...
							<div class="flex items-center gap-4 pt-1">
								<ProcessControls processType="remote_stage_launcher" status={remoteStageLauncherStatus} onStart={startRemoteStageLauncher} onStop={(options) => stopProcess('remote_stage_launcher', options)} />
							</div>
				...
				<div class="space-y-3 pt-2">
					...
					<div class="flex items-center gap-4 pt-1">
						<ProcessControls processType="remote_stage_server" status={remoteStageServerStatus} onStart={startRemoteStageServer} onStop={(options) => stopProcess('remote_stage_server', options)} />
					</div>
