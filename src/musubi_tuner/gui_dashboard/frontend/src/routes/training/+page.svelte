						...
						{#if t.model_type === 'minimax_h3'}
							...
							{#if $advancedMode}
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
									...
									<FormSelect fieldPath="training.h3_guidance_loss_form" value={t.h3_guidance_loss_form || 'normalized'} options={['normalized', 'contrastive', 'additive']} onchange={(e) => update('h3_guidance_loss_form', e.target.value)} tooltip="Contrastive directly supervises the extrapolated guided target; normalized has the same optimum with normalized gradients" />
								...
								<PathInput fieldPath="training.h3_overlay_weights" value={t.h3_overlay_weights || ''} oninput={(e) => update('h3_overlay_weights', e.target.value)} showFiles tooltip="Optional frozen LoRA kept live on top of the base for the whole run: never merged, never trained, never saved into the trained adapter. Works with an INT8 ConvRot base, where merging would have to requantize." />
