					...
					<FormGroup title="Quantization">
						<div class="space-y-2 pt-2">
							...
							<div class="grid grid-cols-3 gap-x-4 gap-y-1">
								...
								<FormToggle label="W4A8" fieldPath="training.w4a8" checked={t.w4a8 ?? false} onchange={(e) => update('w4a8', e.target.checked)} tooltip="4-bit weights with 8-bit activations (int4 container only). Mutually exclusive with W4A4G4, W4A4G8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="W4A4G8" fieldPath="training.w4a4g8" checked={t.w4a4g8 ?? false} onchange={(e) => update('w4a4g8', e.target.checked)} tooltip="4-bit weights/activations with row-wise INT8 grad-output; int4 container only. Mutually exclusive with W4A4G4, W4A8 and FP8/NF4/INT8 base modes." />
								<FormToggle label="INT4 No MSE Clip" fieldPath="training.int4_convrot_no_mse_clip" checked={t.int4_convrot_no_mse_clip ?? false} onchange={(e) => update('int4_convrot_no_mse_clip', e.target.checked)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} tooltip="Use plain absmax row scales instead of the MSE-selected clipping scale during on-the-fly INT4 ConvRot quantization." />
							</div>
							<div class="grid grid-cols-3 gap-2">
							...
							<div class="grid grid-cols-3 gap-2">
								<FormField label="INT4 Groupsize" fieldPath="training.int4_convrot_groupsize" value={t.int4_convrot_groupsize || 'auto'} oninput={(e) => update('int4_convrot_groupsize', e.target.value || 'auto')} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} placeholder="auto" tooltip="INT4 ConvRot group size or comma list. Auto tries 256,64,16." />
								<FormField label="INT4 Quality Report" fieldPath="training.int4_convrot_quality_report" value={t.int4_convrot_quality_report || ''} oninput={(e) => update('int4_convrot_quality_report', e.target.value)} disabled={!(t.w4a4g4 || t.w4a8 || t.w4a4g8)} placeholder="Optional JSON path" tooltip="Write per-layer reconstruction metrics during on-the-fly INT4 ConvRot quantization." />
