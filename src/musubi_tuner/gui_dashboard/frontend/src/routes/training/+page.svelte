							...
							{#if $advancedMode}
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
									<FormField type="number" fieldPath="training.h3_guidance_distillation_scale" value={t.h3_guidance_distillation_scale ?? ''} oninput={(e) => update('h3_guidance_distillation_scale', e.target.value ? Number(e.target.value) : null)} min={1.01} step="0.1" placeholder="Optional" tooltip="Enable two-pass guidance-consistent training at this target scale" />
										<FormField type="number" fieldPath="training.h3_guidance_distillation_probability" value={t.h3_guidance_distillation_probability ?? 1} oninput={(e) => update('h3_guidance_distillation_probability', Number(e.target.value || 1))} min={0.01} max={1} step="0.05" disabled={!(t.h3_guidance_distillation_scale > 1)} tooltip="Run the guidance empty forward on this fraction of steps. Active losses are scaled automatically by 1/probability; 0.25–0.5 is the recommended faster range." invalid={fieldInvalid('training.h3_guidance_distillation_probability')} error={fieldError('training.h3_guidance_distillation_probability')} />
