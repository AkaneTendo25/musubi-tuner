<script>
	...
	onMount(() => {
		preloadLogsIfActive(['cache_dino']);
		const logInterval = startLogPolling(['cache_dino'], 1000);
...
{:else}
	<div class="space-y-5">
		...
		<div style="background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); position: relative; overflow: hidden;">
			...
			<div class="p-5 pt-0">
				<div class="grid grid-cols-4 gap-2">
					<FormField fieldPath="training.differential_guidance_scale" label="Scale" type="number" value={$projectConfig?.training?.differential_guidance_scale ?? 3.0} oninput={(e) => updateTraining('differential_guidance_scale', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Default 3.0. 1.0 keeps the original target; >1 strengthens the target delta; 0-1 softens it." />
					<FormSelect fieldPath="training.differential_guidance_schedule" label="Schedule" value={$projectConfig?.training?.differential_guidance_schedule || 'constant'} onchange={(e) => updateTraining('differential_guidance_schedule', e.target.value)} options={[{value:'constant',label:'Constant'}, {value:'linear',label:'Linear'}, {value:'cosine',label:'Cosine'}]} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Constant preserves legacy behavior. Linear/cosine enable warmup and decay." />
					<FormField fieldPath="training.differential_guidance_start_scale" label="Start" type="number" value={$projectConfig?.training?.differential_guidance_start_scale ?? 1.0} oninput={(e) => updateTraining('differential_guidance_start_scale', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance || ($projectConfig?.training?.differential_guidance_schedule || 'constant') === 'constant'} tooltip="Scale at the beginning of warmup." />
					<FormField fieldPath="training.differential_guidance_end_scale" label="End" type="number" value={$projectConfig?.training?.differential_guidance_end_scale ?? 1.0} oninput={(e) => updateTraining('differential_guidance_end_scale', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance || ($projectConfig?.training?.differential_guidance_schedule || 'constant') === 'constant'} tooltip="Scale after decay." />
				</div>
				<div class="grid grid-cols-3 gap-2 mt-2">
					<FormField fieldPath="training.differential_guidance_warmup_steps" label="Warmup" type="number" value={$projectConfig?.training?.differential_guidance_warmup_steps ?? 0} oninput={(e) => updateTraining('differential_guidance_warmup_steps', Number(e.target.value))} step="1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Steps from Start to Scale." />
					<FormField fieldPath="training.differential_guidance_hold_steps" label="Hold" type="number" value={$projectConfig?.training?.differential_guidance_hold_steps ?? 0} oninput={(e) => updateTraining('differential_guidance_hold_steps', Number(e.target.value))} step="1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Steps held at peak Scale." />
					<FormField fieldPath="training.differential_guidance_decay_steps" label="Decay" type="number" value={$projectConfig?.training?.differential_guidance_decay_steps ?? 0} oninput={(e) => updateTraining('differential_guidance_decay_steps', Number(e.target.value))} step="1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Steps from Scale to End." />
				</div>
				<div class="grid grid-cols-4 gap-2 mt-2">
					<FormSelect fieldPath="training.differential_guidance_timestep_mode" label="Timestep" value={$projectConfig?.training?.differential_guidance_timestep_mode || 'none'} onchange={(e) => updateTraining('differential_guidance_timestep_mode', e.target.value)} options={[{value:'none',label:'Uniform'}, {value:'mid',label:'Mid sigma'}, {value:'snr',label:'Low noise'}, {value:'inverse_snr',label:'High noise'}]} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Shape guidance strength across diffusion noise levels." />
					<FormField fieldPath="training.differential_guidance_timestep_floor" label="Floor" type="number" value={$projectConfig?.training?.differential_guidance_timestep_floor ?? 1.0} oninput={(e) => updateTraining('differential_guidance_timestep_floor', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Scale used where the selected timestep weight is zero." />
					<FormField fieldPath="training.differential_guidance_residual_clip" label="RMS Clip" type="number" value={$projectConfig?.training?.differential_guidance_residual_clip ?? 0.0} oninput={(e) => updateTraining('differential_guidance_residual_clip', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Clip residuals at this RMS multiple; zero disables." />
					<FormToggle fieldPath="training.differential_guidance_normalize_residual" label="RMS Normalize" checked={$projectConfig?.training?.differential_guidance_normalize_residual ?? false} onchange={(e) => updateTraining('differential_guidance_normalize_residual', e.target.checked)} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Normalize each sample residual by its RMS." />
				</div>
				<div class="grid grid-cols-4 gap-2 mt-2">
					<FormField fieldPath="training.differential_guidance_adaptive_target_norm" label="Target Grad" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_target_norm ?? 0.0} oninput={(e) => updateTraining('differential_guidance_adaptive_target_norm', Number(e.target.value))} step="0.01" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Target video gradient norm; zero disables. Do not combine with Target V/A." />
					<FormField fieldPath="training.differential_guidance_adaptive_target_ratio" label="Target V/A" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_target_ratio ?? 0.0} oninput={(e) => updateTraining('differential_guidance_adaptive_target_ratio', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Target video/audio gradient ratio for AV training; zero disables." />
					<FormField fieldPath="training.differential_guidance_adaptive_ema" label="EMA" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_ema ?? 0.95} oninput={(e) => updateTraining('differential_guidance_adaptive_ema', Number(e.target.value))} step="0.01" min={0} max={0.9999} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Gradient feedback smoothing." />
					<FormField fieldPath="training.differential_guidance_adaptive_rate" label="Rate" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_rate ?? 0.1} oninput={(e) => updateTraining('differential_guidance_adaptive_rate', Number(e.target.value))} step="0.01" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Adaptive response rate." />
				</div>
				<div class="grid grid-cols-2 gap-2 mt-2">
					<FormField fieldPath="training.differential_guidance_adaptive_min" label="Adaptive Min" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_min ?? 0.25} oninput={(e) => updateTraining('differential_guidance_adaptive_min', Number(e.target.value))} step="0.05" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Minimum adaptive multiplier." />
					<FormField fieldPath="training.differential_guidance_adaptive_max" label="Adaptive Max" type="number" value={$projectConfig?.training?.differential_guidance_adaptive_max ?? 4.0} oninput={(e) => updateTraining('differential_guidance_adaptive_max', Number(e.target.value))} step="0.1" min={0} disabled={!$projectConfig?.training?.differential_guidance} tooltip="Maximum adaptive multiplier." />
				</div>
			</div>
		</div>
		...
		<div style="background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); position: relative; overflow: hidden;">
			...
			<div class="p-5 pt-0 space-y-3">
				<div class="p-3" style="background: var(--bg-elevated); border-radius: var(--radius-sm); border: 1px solid var(--border-subtle);">
					...
					<div class="grid grid-cols-3 gap-2 mb-2">
						<FormSelect fieldPath="training.self_flow_teacher_mode" value={$projectConfig?.training?.self_flow_teacher_mode ?? 'ema'} onchange={(e) => updateTraining('self_flow_teacher_mode', e.target.value)} options={[{value: 'ema', label: 'EMA (canonical)'}, {value: 'partial_ema', label: 'Partial EMA'}, {value: 'base', label: 'Base consistency'}]} tooltip="ema: canonical Self-Flow EMA teacher over all trainable transformer parameters. partial_ema: lower-memory approximation over the teacher block. base: frozen pretrained consistency teacher; not canonical Self-Flow." />
					...
					<div class="grid grid-cols-3 gap-2">
						<FormField type="number" fieldPath="training.self_flow_student_block_ratio" value={$projectConfig?.training?.self_flow_student_block_ratio ?? ''} oninput={(e) => updateTraining('self_flow_student_block_ratio', e.target.value === '' ? null : Number(e.target.value))} step="0.05" min={0} max={1} placeholder="Use index" tooltip="Optional ratio-based student block: floor(ratio × depth). Leave blank to use the student block index." />
						<FormField type="number" fieldPath="training.self_flow_teacher_block_ratio" value={$projectConfig?.training?.self_flow_teacher_block_ratio ?? ''} oninput={(e) => updateTraining('self_flow_teacher_block_ratio', e.target.value === '' ? null : Number(e.target.value))} step="0.05" min={0} max={1} placeholder="Use index" tooltip="Optional ratio-based teacher block: ceil(ratio × depth). Leave blank to use the teacher block index." />
				...
				<div class="p-3" style="background: var(--bg-elevated); border-radius: var(--radius-sm); border: 1px solid var(--border-subtle);">
					...
					<div class="grid grid-cols-3 gap-2">
						...
						<FormField type="number" fieldPath="training.self_flow_temporal_max_steps" value={$projectConfig?.training?.self_flow_temporal_max_steps ?? 0} oninput={(e) => updateTraining('self_flow_temporal_max_steps', Number(e.target.value))} min={0} tooltip="Step at which decay reaches its end weight (0 = no decay)" />
					</div>
					<div class="grid grid-cols-3 gap-2 mt-2">
						<FormField type="number" fieldPath="training.self_flow_schedule_end_weight" value={$projectConfig?.training?.self_flow_schedule_end_weight ?? 0.0} oninput={(e) => updateTraining('self_flow_schedule_end_weight', Number(e.target.value))} min={0} max={1} step="0.01" tooltip="Final schedule multiplier after temporal_max_steps." />
						<FormField type="number" fieldPath="training.self_flow_schedule_power" value={$projectConfig?.training?.self_flow_schedule_power ?? 1.0} oninput={(e) => updateTraining('self_flow_schedule_power', Number(e.target.value))} min={0.000001} step="0.1" tooltip="Exponent used by polynomial decay." />
						<FormField type="number" fieldPath="training.self_flow_schedule_cutoff_step" value={$projectConfig?.training?.self_flow_schedule_cutoff_step ?? 0} oninput={(e) => updateTraining('self_flow_schedule_cutoff_step', Number(e.target.value))} min={0} tooltip="Hard-disable Self-Flow at this step (0 = disabled)." />
					</div>
					<div class="grid grid-cols-3 gap-2 mt-2">
						<FormField type="number" fieldPath="training.self_flow_similarity_cutoff" value={$projectConfig?.training?.self_flow_similarity_cutoff ?? ''} oninput={(e) => updateTraining('self_flow_similarity_cutoff', e.target.value ? Number(e.target.value) : null)} min={-1} max={1} step="0.01" placeholder="Disabled" tooltip="Disable the loss when EMA cosine reaches this threshold." />
						<FormField type="number" fieldPath="training.self_flow_similarity_ema_decay" value={$projectConfig?.training?.self_flow_similarity_ema_decay ?? 0.99} oninput={(e) => updateTraining('self_flow_similarity_ema_decay', Number(e.target.value))} min={0} max={0.999999} step="0.001" tooltip="EMA decay for the similarity cutoff signal." />
						<FormSelect fieldPath="training.self_flow_similarity_cutoff_mode" value={$projectConfig?.training?.self_flow_similarity_cutoff_mode ?? 'permanent'} onchange={(e) => updateTraining('self_flow_similarity_cutoff_mode', e.target.value)} options={[{value: 'permanent', label: 'Permanent cutoff'}, {value: 'recoverable', label: 'Recoverable cutoff'}]} tooltip="Permanent latches once reached; recoverable keeps measuring and resumes if similarity falls." />
					</div>
				</div>
			...
			<div class="p-5 pt-0 space-y-3">
				<div class="p-3" style="background: var(--bg-elevated); border-radius: var(--radius-sm); border: 1px solid var(--border-subtle);">
					...
					<div class="grid grid-cols-3 gap-2">
						...
						<FormSelect fieldPath="training.hfato_interpolation" value={$projectConfig?.training?.hfato_interpolation || 'trilinear'} onchange={(e) => updateTraining('hfato_interpolation', e.target.value)} options={[{value: 'trilinear', label: 'Trilinear'}, {value: 'nearest', label: 'Nearest'}, {value: 'bilinear', label: 'Bilinear'}, {value: 'bicubic', label: 'Bicubic'}]} tooltip="Interpolation mode for downsample-upsample. ViBe uses trilinear with align_corners=False (default)" />
		...
		<div style="background: var(--bg-surface); border: 1px solid var(--border-subtle); border-radius: var(--radius-md); position: relative; overflow: hidden;">
			...
			<div class="p-5 space-y-4">
				...
				<div class="p-3" style="background: var(--bg-elevated); border-radius: var(--radius-sm); border: 1px solid var(--border-subtle);">
					...
					<p class="text-[11px] leading-relaxed mb-2" style="color: var(--text-muted);">At each supervised state the adapter is matched to a frozen concept teacher LoRA under the trigger and to the untouched base under the trigger-free caption. Needs the same trigger-free text cache as DOP (trigger set, caption mode bare).</p>
					<div class="grid grid-cols-2 gap-2">
						<FormField fieldPath="training.h3_two_teacher_weights" value={$projectConfig?.training?.h3_two_teacher_weights || ''} oninput={(e) => updateTraining('h3_two_teacher_weights', e.target.value)} placeholder="/path/to/teacher.safetensors" tooltip="LoRA of the concept teacher; frozen, live only on the teacher's forward" />
				...
				<div class="p-3" style="background: var(--bg-elevated); border-radius: var(--radius-sm); border: 1px solid var(--border-subtle);">
					...
					<div class="grid grid-cols-5 gap-x-4 gap-y-1 mb-2">
						...
						<FormToggle fieldPath="training.audio_metrics_clap_fad" checked={$projectConfig?.training?.audio_metrics_clap_fad ?? false} onchange={(e) => updateTraining('audio_metrics_clap_fad', e.target.checked)} disabled={!$projectConfig?.training?.audio_metrics} tooltip="Dataset-level FAD-CLAP with full-rank covariance safeguards" />
						<FormToggle fieldPath="training.audio_metrics_av_onset_alignment" checked={$projectConfig?.training?.audio_metrics_av_onset_alignment ?? false} onchange={(e) => updateTraining('audio_metrics_av_onset_alignment', e.target.checked)} disabled={!$projectConfig?.training?.audio_metrics} tooltip="Correlation between audio onsets and decoded-video motion at sampling time" />
						<FormToggle fieldPath="training.audio_metrics_av_desync" checked={$projectConfig?.training?.audio_metrics_av_desync ?? false} onchange={(e) => updateTraining('audio_metrics_av_desync', e.target.checked)} disabled={!$projectConfig?.training?.audio_metrics} tooltip="Synchformer mean absolute AV offset and derived sync score" />
					</div>
					<div class="grid grid-cols-3 gap-2">
						...
						<FormField type="number" fieldPath="training.audio_metrics_clap_fad_min_samples" value={$projectConfig?.training?.audio_metrics_clap_fad_min_samples ?? 513} oninput={(e) => updateTraining('audio_metrics_clap_fad_min_samples', Number(e.target.value))} step="1" min={2} disabled={!$projectConfig?.training?.audio_metrics_clap_fad} tooltip="Minimum paired samples. Runtime also requires embedding dimension + 1 and full covariance rank." />
						<FormField type="number" fieldPath="training.audio_metrics_av_desync_max_length_s" value={$projectConfig?.training?.audio_metrics_av_desync_max_length_s ?? 8} oninput={(e) => updateTraining('audio_metrics_av_desync_max_length_s', Number(e.target.value))} step="0.5" min={0.5} disabled={!$projectConfig?.training?.audio_metrics_av_desync} tooltip="Maximum seconds scored by Synchformer per sample." />
					</div>
					<div class="grid grid-cols-2 gap-2 mt-2">
						<PathInput fieldPath="training.audio_metrics_av_desync_checkpoint" value={$projectConfig?.training?.audio_metrics_av_desync_checkpoint || ''} oninput={(e) => updateTraining('audio_metrics_av_desync_checkpoint', e.target.value)} showFiles disabled={!$projectConfig?.training?.audio_metrics_av_desync} tooltip="Synchformer state-dict checkpoint used only for validation." />
						<FormSelect fieldPath="training.audio_metrics_av_desync_device" value={$projectConfig?.training?.audio_metrics_av_desync_device ?? 'cpu'} options={['cpu', 'cuda', 'auto']} onchange={(e) => updateTraining('audio_metrics_av_desync_device', e.target.value)} disabled={!$projectConfig?.training?.audio_metrics_av_desync} tooltip="CPU is safest for VRAM; CUDA is faster when memory permits." />
					</div>
					<div class="mt-2">
