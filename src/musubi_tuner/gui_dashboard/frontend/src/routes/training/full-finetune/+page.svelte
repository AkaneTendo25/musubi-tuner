		...
		<FormGroup title="LTX-2 Performance">
			<div class="space-y-2 pt-2">
				<div class="grid grid-cols-2 gap-x-4 gap-y-1">
					...
					<FormField label="XM noise candidates" type="number" fieldPath="full_finetune.ltx2_xm_k" value={t.ltx2_xm_k ?? 1} oninput={(e) => update('ltx2_xm_k', numberOrNull(e.target.value) ?? 1)} min={1} max={32} step={1} tooltip="Forward Explorative Modeling candidates per sample. 1 disables XM; larger values add no-gradient scoring forwards before the selected training forward." />
				</div>
				<div class="grid grid-cols-3 gap-2">
