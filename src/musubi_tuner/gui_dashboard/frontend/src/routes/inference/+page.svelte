...
{:else}
	<div class="space-y-4">
					...
					<div class="space-y-2 pt-2">
						...
						{#if s.model_type === 'minimax_h3'}
							...
							<div class="grid grid-cols-2 gap-2">
								...
								<PathInput fieldPath="inference.h3_last_frame" value={s.h3_last_frame || ''} oninput={(e) => update('h3_last_frame', e.target.value)} showFiles tooltip="Optional last-frame image" />
							</div>
							<div class="grid grid-cols-3 gap-2">
								<PathInput fieldPath="inference.h3_reference_image" value={s.h3_reference_image || ''} oninput={(e) => update('h3_reference_image', e.target.value)} showFiles tooltip="Optional reference image" />
							...
							<FormField type="number" fieldPath="inference.h3_reference_image_short_edge" value={s.h3_reference_image_short_edge ?? 2048} oninput={(e) => update('h3_reference_image_short_edge', Number(e.target.value || 2048))} min={64} step={8} tooltip="Must match the reference size used to train the LoRA" />
			...
			<div class="space-y-3">
				{#if s.model_type === 'minimax_h3'}
					<FormGroup title="Generation">
						<div class="space-y-2 pt-2">
							<FormField type="number" fieldPath="inference.seed" value={s.seed ?? ''} oninput={(e) => update('seed', e.target.value ? Number(e.target.value) : null)} placeholder="42" tooltip="Random seed" />
						</div>
					</FormGroup>
				{:else}
				<FormGroup title="Generation">
		...
		<!-- Controls -->
		<div class="py-4">
			<ProcessControls processType="inference" status={inferenceStatus} onStart={() => startProcess('inference')} onStop={(options) => stopProcess('inference', options)} />
		</div>
