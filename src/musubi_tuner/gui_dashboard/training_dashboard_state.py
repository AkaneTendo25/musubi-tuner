...
def get_latest_local_autoresume_state(config: ProjectConfig, process_type: str = "training") -> Optional[Path]:
...
def has_local_autoresume_state(config: ProjectConfig, process_type: str = "training") -> bool:
    return get_latest_local_autoresume_state(config, process_type) is not None
