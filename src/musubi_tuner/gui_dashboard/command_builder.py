...
def _build_h3_cache_text_cmd(config: ProjectConfig) -> list[str]:
    ...
    cmd = [
        sys.executable,
        "-u",
        _find_script("minimax_h3_cache_text_encoder_outputs.py"),
        "--dataset_config",
        str(toml_path),
        "--text_encoder",
        c.h3_text_encoder,
...
def _build_h3_inference_cmd(config: ProjectConfig) -> list[str]:
    ...
    for flag, raw in (
        ...
        ("--reference_audio", s.h3_reference_audio),
    ):
...
def _build_h3_training_cmd(config: ProjectConfig) -> list[str]:
    ...
    if t.fp8_base or t.fp8_scaled:
        ...
        if t.h3_fp8_quantization_mode != "block":
            cmd += ["--h3_fp8_quantization_mode", t.h3_fp8_quantization_mode]
    if t.int8_convrot_base:
        cmd.append("--int8_convrot_base")
    ...
    if t.h3_video_loss_weight != 1.0:
        cmd += ["--h3_video_loss_weight", str(t.h3_video_loss_weight)]
    ...
    if t.h3_overlay_weights:
        ...
        if t.h3_overlay_training_only:
            ...
            if t.h3_validate_without_overlay:
                cmd.append("--h3_validate_without_overlay")
...
def build_cache_latents_cmd(config: ProjectConfig) -> list[str]:
    ...
    if config.caching.model_type == "minimax_h3":
        return _build_h3_cache_latents_cmd(config)
...
def build_cache_text_cmd(config: ProjectConfig) -> list[str]:
    ...
    if config.caching.model_type == "minimax_h3":
        return _build_h3_cache_text_cmd(config)
    toml_path = export_dataset_toml(config)
    c = config.caching
    t = config.training
    ltx2_checkpoint = _effective_ltx2_checkpoint(config, c.ltx2_checkpoint)
...
def build_inference_cmd(config: ProjectConfig) -> list[str]:
    ...
    if config.inference.model_type == "minimax_h3":
        return _build_h3_inference_cmd(config)
    s = config.inference
...
def build_training_cmd(config: ProjectConfig) -> list[str]:
    ...
    if config.training.model_type == "minimax_h3":
        return _build_h3_training_cmd(config)
    toml_path = export_dataset_toml(config)
    t = config.training
...
def build_full_finetune_cmd(config: ProjectConfig) -> list[str]:
    ...
    if t.log_cuda_memory_every_n_steps is not None:
        cmd += ["--log_cuda_memory_every_n_steps", str(t.log_cuda_memory_every_n_steps)]
    _append_resume_args(cmd, config, t, "full_finetune")
    if t.reset_optimizer:
        cmd.append("--reset_optimizer")
    if t.reset_optimizer_params:
        cmd.append("--reset_optimizer_params")
