...
from musubi_tuner.cosmos3.cosmos_framework.utils.flags import DEVICE
from musubi_tuner.cosmos3.cosmos_framework.utils import log
from musubi_tuner.cosmos3.cosmos_framework.data.vfm.utils import VIDEO_RES_SIZE_INFO
from musubi_tuner.cosmos3.cosmos_framework.model.vfm.tokenizers.interface import VideoTokenizerInterface
from musubi_tuner.cosmos3.cosmos_framework.utils.vfm.data_utils import get_vision_data_resolution
...
def _remap_diffusers_wan_vae_resnet_key(subkey: str) -> str:
    resnet_key_map = {
    ...
    if subkey not in resnet_key_map:
        raise KeyError(f"Unsupported Diffusers Wan VAE residual key: {subkey}")
    return resnet_key_map[subkey]


def _remap_diffusers_wan_vae_state_dict(ckpt: dict[str, torch.Tensor], native_keys: set[str]) -> dict[str, torch.Tensor]:
    """
    Convert Hugging Face/Diffusers AutoencoderKLWan checkpoint keys to this
    native WanVAE_ layout. Cosmos3-Nano official snapshots store the VAE under
    these Diffusers-style names even when the rest of the pipeline is loaded
    natively.
    """
    direct_key_map = {
    ...
    mid_resnet_index_map = {"0": "0", "1": "2"}
    remapped: dict[str, torch.Tensor] = {}

    for key, tensor in ckpt.items():
        new_key = None

        for src_prefix, dst_prefix in direct_key_map.items():
        ...
        if new_key is None:
            for prefix in ("encoder", "decoder"):
                mid_resnet_prefix = f"{prefix}.mid_block.resnets."
                if key.startswith(mid_resnet_prefix):
                    rest = key[len(mid_resnet_prefix) :]
                    block_idx, subkey = rest.split(".", 1)
                    if block_idx not in mid_resnet_index_map:
                        raise KeyError(f"Unsupported Diffusers Wan VAE mid-block residual index: {key}")
                    new_key = (
                        f"{prefix}.middle.{mid_resnet_index_map[block_idx]}."
                        f"{_remap_diffusers_wan_vae_resnet_key(subkey)}"
                    )
                    break

                mid_attention_prefix = f"{prefix}.mid_block.attentions.0."
                if key.startswith(mid_attention_prefix):
                    new_key = f"{prefix}.middle.1." + key[len(mid_attention_prefix) :]
                    break

        if new_key is None and key.startswith("encoder.down_blocks."):
            rest = key[len("encoder.down_blocks.") :]
            block_idx, rest = rest.split(".", 1)
            if rest.startswith("resnets."):
                rest = rest[len("resnets.") :]
                resnet_idx, subkey = rest.split(".", 1)
                new_key = (
                    f"encoder.downsamples.{block_idx}.downsamples.{resnet_idx}."
                    f"{_remap_diffusers_wan_vae_resnet_key(subkey)}"
                )
            elif rest.startswith("downsampler."):
                new_key = f"encoder.downsamples.{block_idx}.downsamples.2." + rest[len("downsampler.") :]

        if new_key is None and key.startswith("decoder.up_blocks."):
            ...
            if rest.startswith("resnets."):
                ...
                resnet_idx, subkey = rest.split(".", 1)
                new_key = (
                    f"decoder.upsamples.{block_idx}.upsamples.{resnet_idx}."
                    f"{_remap_diffusers_wan_vae_resnet_key(subkey)}"
                )
            elif rest.startswith("upsampler."):
                new_key = f"decoder.upsamples.{block_idx}.upsamples.3." + rest[len("upsampler.") :]

        if new_key is None:
            raise KeyError(f"Unsupported Diffusers Wan VAE key: {key}")
        if new_key not in native_keys:
            raise KeyError(f"Diffusers Wan VAE key {key} remapped to unknown native key {new_key}")
        if new_key in remapped:
            raise KeyError(f"Diffusers Wan VAE key collision at native key {new_key}")
        remapped[new_key] = tensor

    missing = native_keys - set(remapped)
    extra = set(remapped) - native_keys
    if missing or extra:
        raise RuntimeError(f"Incomplete Diffusers Wan VAE remap, missing={sorted(missing)}, extra={sorted(extra)}")
    return remapped
...
def _video_vae(
    ...
    if pretrained_path is None:
        ...
        if "://" in pretrained_path:
            raise ValueError("WanVAE in the local Cosmos package only supports local checkpoint files.")
        if object_store_credential_path_pretrained:
            raise ValueError("Object-store VAE checkpoints are not supported by this local Cosmos package.")

        model.to_empty(device=device)
        if os.path.splitext(pretrained_path)[1] == ".safetensors":
            ckpt = load_file(pretrained_path, device=str(device))
        else:
            ckpt = torch.load(pretrained_path, map_location=str(device), weights_only=True)

        log.info(f"loading {pretrained_path}")
        if "encoder.conv_in.weight" in ckpt or "quant_conv.weight" in ckpt:
            log.info("detected Diffusers Wan VAE checkpoint; remapping keys to native WanVAE_ layout")
            ckpt = _remap_diffusers_wan_vae_state_dict(ckpt, set(model.state_dict()))
...
class WanVAE:
    ...
    def _refresh_runtime_attrs(self):
        try:
        ...
        if self.is_amp and self.device.type == "cuda":
            self.context = torch.amp.autocast("cuda", dtype=self.dtype)
        else:
            self.context = nullcontext()

    def to(self, *args, **kwargs):
        self.model = self.model.to(*args, **kwargs)
        self.scale = tuple(scale.to(*args, **kwargs) for scale in self.scale)
        self._refresh_runtime_attrs()
        return self
