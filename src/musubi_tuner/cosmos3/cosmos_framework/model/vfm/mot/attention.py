...
try:
    ...
    diffusers_dispatch_attention_fn = None

from musubi_tuner.cosmos3.cosmos_framework.model.attention import (
    attention,
    merge_attentions,
    multi_dimensional_attention_varlen,
)
from musubi_tuner.cosmos3.cosmos_framework.model.attention.masks import CausalType
from musubi_tuner.cosmos3.cosmos_framework.model.vfm.utils.memory import KVToStore, MemoryValue
...
def _pack_to_batch(tokens: torch.Tensor, cu_seqlens: torch.Tensor, max_seqlen: int) -> torch.Tensor:
    ...
    cu = cu_seqlens.detach().cpu().long().tolist()
    out = tokens.new_zeros(batch, max_seqlen, *tokens.shape[1:])
    for index in range(batch):
        length = cu[index + 1] - cu[index]
        out[index, :length] = tokens[cu[index] : cu[index + 1]]
    return out


def _batch_to_pack(batched: torch.Tensor, cu_seqlens: torch.Tensor) -> torch.Tensor:
    cu = cu_seqlens.detach().cpu().long().tolist()
    return torch.cat([batched[index, : cu[index + 1] - cu[index]] for index in range(len(cu) - 1)], dim=0)
...
def _kv_padding_mask(cu_seqlens: torch.Tensor, max_seqlen: int, dtype: torch.dtype, device: torch.device):
    ...
    for index in range(batch):
        ...
        if length < max_seqlen:
            mask[index, 0, 0, length:] = float("-inf")
...
def _packed_sdpa(
    ...
    *,
    ...
    if diffusers_dispatch_attention_fn is not None:
        return diffusers_dispatch_attention_fn(
            query,
            key,
            value,
            attn_mask=attn_mask,
...
def two_way_attention(
    ...
    causal_res = _packed_sdpa(
        _pack_to_batch(causal_q, causal_q_offsets, max_causal),
        _pack_to_batch(causal_k, causal_q_offsets, max_causal),
        _pack_to_batch(causal_v, causal_q_offsets, max_causal),
        is_causal=True,
    )
    causal_out = _batch_to_pack(causal_res, causal_q_offsets).flatten(-2, -1)

    all_k = get_all_seq(packed_key_states)
    all_v = get_all_seq(packed_value_states)
    full_res = _packed_sdpa(
        _pack_to_batch(full_q, full_q_offsets, max_full),
        _pack_to_batch(all_k, sample_offsets, max_sample),
        _pack_to_batch(all_v, sample_offsets, max_sample),
        attn_mask=_kv_padding_mask(sample_offsets, max_sample, full_q.dtype, full_q.device),
    )
    full_out = _batch_to_pack(full_res, full_q_offsets).flatten(-2, -1)
