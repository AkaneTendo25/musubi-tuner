...
from musubi_tuner.cosmos3.cosmos_framework.model.attention.backends import choose_backend, choose_multi_dim_backend
from musubi_tuner.cosmos3.cosmos_framework.model.attention.checks import (
...
from musubi_tuner.cosmos3.cosmos_framework.model.attention.flash2 import flash2_attention
from musubi_tuner.cosmos3.cosmos_framework.model.attention.flash3 import flash3_attention
from musubi_tuner.cosmos3.cosmos_framework.model.attention.masks import CausalType
from musubi_tuner.cosmos3.cosmos_framework.model.attention.natten import natten_attention, natten_multi_dim_attention
from musubi_tuner.cosmos3.cosmos_framework.model.attention.utils.environment import filter_attention_merge_backends
from musubi_tuner.cosmos3.cosmos_framework.model.attention.utils.safe_ops import log
...
_TORCH_ATTENTION_FALLBACK_WARNED = False
_TORCH_MERGE_FALLBACK_WARNED = False
...
def _causal_mask(
    ...
    if causal_type == CausalType.BottomRight:
        return q_idx + kv_len - q_len >= kv_idx
    return q_idx >= kv_idx
...
def _torch_sdpa_attention(
    ...
    output = torch.empty((*query.shape[:-1], value.shape[-1]), dtype=query.dtype, device=query.device)
    lse = torch.empty(query.shape[:-1], dtype=torch.float32, device=query.device) if return_lse else None

    if cumulative_seqlen_Q is not None:
        assert cumulative_seqlen_KV is not None
        q_offsets = cumulative_seqlen_Q.detach().cpu().long().tolist()
        kv_offsets = cumulative_seqlen_KV.detach().cpu().long().tolist()
        segments = [
            (0, q_offsets[index], q_offsets[index + 1], kv_offsets[index], kv_offsets[index + 1])
            for index in range(len(q_offsets) - 1)
        ]
    else:
        segments = [(batch_index, 0, query.shape[1], 0, key.shape[1]) for batch_index in range(query.shape[0])]

    for batch_idx, q_start, q_end, kv_start, kv_end in segments:
        if q_end == q_start:
            continue

        q = query[batch_idx : batch_idx + 1, q_start:q_end].transpose(1, 2)
        k = key[batch_idx : batch_idx + 1, kv_start:kv_end].transpose(1, 2)
        v = value[batch_idx : batch_idx + 1, kv_start:kv_end].transpose(1, 2)

        attn_mask = None
        if is_causal:
            attn_mask = _causal_mask(q_end - q_start, kv_end - kv_start, causal_type, query.device).view(
                1, 1, q_end - q_start, kv_end - kv_start
            )
        ...
        output[batch_idx, q_start:q_end] = segment_output.transpose(1, 2)
        if lse is not None:
            lse[batch_idx, q_start:q_end] = _attention_lse(q, k, scale, attn_mask)[0]
...
def _merge_attentions_torch(outputs: list[Tensor], lse_tensors: list[Tensor]) -> tuple[Tensor, Tensor]:
    lse_stack = torch.stack([lse.float() for lse in lse_tensors], dim=0)
    merged_lse = torch.logsumexp(lse_stack, dim=0)
    weights = torch.exp(lse_stack - merged_lse.unsqueeze(0)).unsqueeze(-1)
    merged_output = sum(output * weight.to(dtype=output.dtype) for output, weight in zip(outputs, weights))
...
def attention(
    ...
    dimension. Sequence lengths for different batches can be provided in two ways:
        ...
        2. `cumulative_seqlen_{Q,KV}` and `max_seqlen_{Q,KV}` (more efficient):
            ...
            parameters:
                ```python3
                from musubi_tuner.cosmos3.cosmos_framework.model.attention.varlen import generate_varlen_parameters
                (
                    cumulative_seqlen_Q,
    ...
    if compatible_backend is None and backend is None:
        global _TORCH_ATTENTION_FALLBACK_WARNED
        if not _TORCH_ATTENTION_FALLBACK_WARNED:
            log.warning(
                "Could not find a compatible Flash/NATTEN attention backend; falling back to torch SDPA."
            )
            _TORCH_ATTENTION_FALLBACK_WARNED = True
        return _torch_sdpa_attention(
            query=query,
            key=key,
            value=value,
...
def multi_dimensional_attention_varlen(
    ...
    **Requires NATTEN >= 0.21.9.dev0 and either Hopper or Blackwell DC-class architecture**
    ...
    from musubi_tuner.cosmos3.cosmos_framework.model.attention.natten import natten_supported
    ...
    from musubi_tuner.cosmos3.cosmos_framework.model.attention.natten.functions import natten_multi_dim_attention_varlen
...
def merge_attentions(
    ...
    **NOTE: for backpropagation, only two outputs can be merged for now.**
    ...
    global _TORCH_MERGE_FALLBACK_WARNED
    if "natten" not in allowed_backends:
        if not _TORCH_MERGE_FALLBACK_WARNED:
            log.warning(
                "merge_attentions NATTEN backend is disabled; falling back to torch logsumexp merge."
            )
            _TORCH_MERGE_FALLBACK_WARNED = True
        return _merge_attentions_torch(outputs, lse_tensors)

    from musubi_tuner.cosmos3.cosmos_framework.model.attention.natten import natten_supported

    if not natten_supported():
        if not _TORCH_MERGE_FALLBACK_WARNED:
            log.warning("merge_attentions NATTEN backend is unavailable; falling back to torch logsumexp merge.")
            _TORCH_MERGE_FALLBACK_WARNED = True
        return _merge_attentions_torch(outputs, lse_tensors)
