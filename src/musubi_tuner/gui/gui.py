        ...
        with gr.Accordion(i18n("acc_model"), open=False):
            ...
            def init_project(path):
                ...
                try:
                    ...
                    os.makedirs(os.path.join(path, "training_audio"), exist_ok=True)
                    ...
                    if settings:
                        msg += " Settings loaded."
                    if new_model == "OpenMOSS-MOVA":
                        msg += (
                            " 'training' and 'training_audio' folders are ready. "
                            "Place videos and matching caption files in 'training', and matching audio files in 'training_audio'."
                        )
                    else:
                        msg += (
                            " 'training' folder ready. Configure the dataset in the 'training' folder. "
                            "Images and caption files (same name as image, extension is '.txt') should be placed in the 'training' folder."
                        )
                    msg += "\n\nプロジェクトが初期化されました。"
                    if settings:
                        msg += "設定が読み込まれました。"
                    if new_model == "OpenMOSS-MOVA":
                        msg += (
                            "'training' と 'training_audio' フォルダが準備されました。"
                            "動画と対応するキャプションは 'training' に、対応する音声は 'training_audio' に配置してください。"
                        )
                    else:
            ...
            def generate_config(project_path, w, h, batch, model_val, vram_val, comfy_val, vae_val, te1_val, te2_val):
                ...
                audio_dir = os.path.join(project_path, "training_audio").replace("\\", "/")
                cache_dir = os.path.join(project_path, "cache").replace("\\", "/")

                if model_val == "OpenMOSS-MOVA":
...
"""
        ...
        with gr.Accordion(i18n("acc_preprocessing"), open=False):
            ...
            def validate_models_dir(path, model_arch):
                if not path:
                    return "Please enter a model directory. / モデルディレクトリを入力してください。"

                required_subdirs = config_manager.get_required_subdirs(model_arch)
                missing = []
                ...
                if missing:
                    return f"Error: Missing required subdirectories / 必要なサブディレクトリが見つかりません: {', '.join(missing)}"

                return "Valid model directory structure found / 有効なモデルディレクトリ構造が見つかりました。"
            ...
            def set_training_defaults(project_path, comfy_models_dir, model_arch, vram_val):
                ...
                pattern = config_manager.get_cache_pattern(model_arch)
            ...
            def cache_latents(project_path, vae_path_val, te1, te2, model, comfy, w, h, batch, vram_val):
                ...
                elif model == "OpenMOSS-MOVA":
                    script_name = "mova_cache_latents.py"
            ...
            def cache_text_encoder(project_path, te1_path_val, te2_path_val, vae, model, comfy, w, h, batch, vram_val):
                ...
                if model == "Qwen-Image":
                    ...
                    script_name = "mova_cache_text_encoder_outputs.py"
        ...
        def convert_lora_to_comfy(project_path, input_path, output_path, model, comfy, w, h, batch, vae, te1, te2):
            if model != "Z-Image-Turbo":
                yield "Error: Conversion is only supported for Z-Image-Turbo in the current GUI. / 現在のGUIでは変換は Z-Image-Turbo のみ対応しています。"
                return
        ...
        def start_training(
            ...
            if model == "OpenMOSS-MOVA":
                ...
                if os.path.exists(os.path.join(dit, "video_dit_2")):
                    inner_cmd.extend(["--alternate_visual_subfolder", "video_dit_2"])
            ...
            if should_sample_images:
                ...
                templates = {
                    ...
                    "Qwen-Image": {"flow_shift": 2.2, "steps": 20, "cfg_scale": 4.0, "frame_count": None},
                    "Z-Image-Turbo": {"flow_shift": 3.0, "steps": 20, "cfg_scale": 5.0, "frame_count": None},
                    "OpenMOSS-MOVA": {"flow_shift": float(flow_shift), "steps": 20, "cfg_scale": 5.0, "frame_count": 81},
                }
                ...
                line = f"{prompt_str} --w {w_int} --h {h_int} --fs {template['flow_shift']} --s {template['steps']} --l {template['cfg_scale']} --d 1234"
                if neg_str:
                    line += f" --n {neg_str}"
                if template["frame_count"] is not None:
                    line += f" --f {template['frame_count']}"
                try:
        ...
        def update_model_info(model):
            if model == "Z-Image-Turbo":
                ...
                return i18n("desc_training_mova")
            return ""
        ...
        validate_models_btn.click(fn=validate_models_dir, inputs=[comfy_models_dir, model_arch], outputs=[models_status])
