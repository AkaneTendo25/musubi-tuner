...
class AudioDataset(BaseDataset):
    ...
    def _dummy_resolution_str(self) -> str:
        return "0001x0001"

    def _dummy_latent_cache_path(self, item_key: str) -> str:
        basename = os.path.splitext(os.path.basename(item_key))[0]
        return os.path.join(
            self.cache_directory,
            f"{basename}_{self._dummy_resolution_str()}_{self.architecture}.safetensors",
        )

    def _extract_item_key(self, name: str) -> str:
        reso = self._dummy_resolution_str()
        if name.endswith(f"_{reso}"):
            return name[: -(len(reso) + 1)]
        return name
        ...
        def aggregate_future(consume_all: bool = False):
            while len(futures) >= num_workers or (consume_all and len(futures) > 0):
                ...
                for future in completed_futures:
                    ...
                    item_key = audio_path
                    item_info = ItemInfo(item_key, caption, (1, 1), (1, 1))
                    item_info.audio_path = audio_path
                    item_info.latent_cache_path = self._dummy_latent_cache_path(item_key)
                    item_info.audio_latent_cache_path = self.get_audio_latent_cache_path_from_latent_cache_path(
                        item_info.latent_cache_path
                    )
                    data.append(item_info)
                    futures.remove(future)
    ...
    def prepare_for_training(self, num_timestep_buckets: Optional[int] = None):
        audio_cache_glob = os.path.join(
            self.cache_directory, f"*_{self.architecture}_audio.safetensors"
        )
        audio_cache_files = glob.glob(audio_cache_glob)

        bucketed_item_info: dict[Union[tuple[int, int], Any], list[ItemInfo]] = {}
        for cache_file in audio_cache_files:
            base = os.path.basename(cache_file)
            suffix = f"_{self.architecture}_audio.safetensors"
            if not base.endswith(suffix):
                continue
            name = base[: -len(suffix)]
            item_key = self._extract_item_key(name)

            latent_cache_path = os.path.join(
                self.cache_directory,
                f"{item_key}_{self._dummy_resolution_str()}_{self.architecture}.safetensors",
            )
            if not os.path.exists(latent_cache_path):
                logger.warning(f"Dummy latent cache file not found: {latent_cache_path}")
                continue
            ...
            item_info = ItemInfo(item_key, "", (1, 1), bucket_reso, latent_cache_path=latent_cache_path)
            item_info.text_encoder_output_cache_path = text_encoder_output_cache_file
            item_info.audio_latent_cache_path = cache_file
