...
from musubi_tuner.ltx_2.model.video_vae.transformer.dsl_kernels import (
    FUSED_CTX_BIAS_BUFFER,
    FUSED_CTX_WEIGHT_BUFFER,
    NA_SOFTMAX_BOUND_BUFFER,
)
from musubi_tuner.ltx_2.model.video_vae.video_vae import ConvVideoDecoder, DiffusionVideoDecoder, VideoDecoder, VideoEncoder
...
def _vae_class_name_from_metadata(metadata: dict) -> str:
    ...
    # SafetensorsModelStateDictLoader.metadata() returns the decoded config
    # directly, while some callers pass the raw metadata wrapper. Accept both.
    config = metadata.get("config", metadata)
    return config.get("vae", {}).get("_class_name", _CONV_VAE_CLASS_NAME)
...
def diffvae_tiling_geometry_from_vae_config(config: dict) -> dict:
    ...
    from musubi_tuner.ltx_2.model.video_vae import diffusion_tiling as dt  # noqa: PLC0415
    from musubi_tuner.ltx_2.model.video_vae import diffusion_video_decoder as dvd  # noqa: PLC0415

    decoder_config = config.get("decoder", config)
    stage_kernels = (
        tuple(tuple(k) for k in decoder_config["stage_kernels"]) if "stage_kernels" in decoder_config else dvd._L_STAGE_KERNELS
    )
    stage_depths = tuple(decoder_config["stage_depths"]) if "stage_depths" in decoder_config else dvd._DIFF_STAGE_DEPTHS_DEFAULT
    upsamples = (
        tuple((tuple(stride), reduction) for stride, reduction in decoder_config["upsamples"])
        if "upsamples" in decoder_config
        else dvd._L_UPSAMPLES
    )
    stage5_kernel = tuple(decoder_config["stage5_kernel"]) if "stage5_kernel" in decoder_config else dvd._DIFF_STAGE5_KERNEL_DEFAULT
...
def _fold_gate_into_linear(param_key: str, value: torch.Tensor, gates: dict[str, torch.Tensor]) -> list[KeyValueOperationResult]:
...
class _Stage4Hop:
    ...
    def fused_results(
        ...
        weight, bias = pack_fused_upsample_ctx_weights(self.upsample_weight, self.upsample_bias, folded_weight, folded_bias)
        return [
