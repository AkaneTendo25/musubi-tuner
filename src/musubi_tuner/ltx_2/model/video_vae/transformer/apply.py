...
from musubi_tuner.ltx_2.model.video_vae.transformer.compiling import CompilationConfig
from musubi_tuner.ltx_2.model.video_vae.transformer.attention import (
    NeighborhoodAttention3D,
    configure_w_chunks,
    natten_available,
)
from musubi_tuner.ltx_2.model.video_vae.transformer.blocks import DiffusionNABlock
from musubi_tuner.ltx_2.model.video_vae.transformer.chunked.block import ChunkedDiffusionNABlock
from musubi_tuner.ltx_2.model.video_vae.transformer.combined.block import CombinedDiffusionNABlock
from musubi_tuner.ltx_2.model.video_vae.transformer.compiling import (
    compile_diffusion_decoder,
    configure_natten_backend,
)
from musubi_tuner.ltx_2.model.video_vae.transformer.config import (
    DiffVAEBlockKind,
    DiffVAEConfig,
    DiffVAEMode,
    NAttentionKind,
)
from musubi_tuner.ltx_2.model.video_vae.transformer.dsl_kernels.block import DSLDiffusionNABlock
from musubi_tuner.ltx_2.model.video_vae.transformer.fallback_na import (
    EagerSdpaAttention,
    TritonNaAttention,
    triton_na_available,
    warn_no_natten,
...
if TYPE_CHECKING:
    from musubi_tuner.ltx_2.model.video_vae.video_vae import DiffusionVideoDecoder
...
def resolve_attention_for_host(cfg: DiffVAEConfig) -> DiffVAEConfig:
    ...
    if cfg.block is DiffVAEBlockKind.COMBINED:
        raise RuntimeError(
            ...
            "(full-volume NA + torch.compile); use chunked_eager or "
            "chunked_compile to fall back to the vendored Triton/eager implementation."
        )
...
def _install_attention(decoder: DiffusionVideoDecoder, cfg: DiffVAEConfig) -> None:
    if cfg.attention is NAttentionKind.BLACKWELL_DSL:
        from musubi_tuner.ltx_2.model.video_vae.transformer.dsl_kernels.apply_dsl import (  # noqa: PLC0415
...
def _diffusion_decoder_matcher(model: torch.nn.Module) -> bool:
    from musubi_tuner.ltx_2.model.video_vae.video_vae import DiffusionVideoDecoder  # noqa: PLC0415
