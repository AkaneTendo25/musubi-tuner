...
from musubi_tuner.ltx_2.model.video_vae.transformer.swiglu import (
    SwiGLUTileSpec,
...
from musubi_tuner.ltx_2.tiling import DimensionInterval
...
def _chunked_residual_modulated_impl(
    ...
    *,
    ...
    if w_up.shape[0] != hidden or w_gate.shape[1] != dim or w_up.shape[1] != dim:
        raise ValueError(f"weight shapes incompatible with x[..., {dim}]: gate={tuple(w_gate.shape)} up={tuple(w_up.shape)}")
