...
from musubi_tuner.ltx_2.devices import cuda_activation_budget_bytes
from musubi_tuner.ltx_2.model.disposable import Disposable
from musubi_tuner.ltx_2.model.transformer.timestep_embedding import PixArtAlphaCombinedTimestepSizeEmbeddings
from musubi_tuner.ltx_2.model.video_vae import diffusion_tiling
from musubi_tuner.ltx_2.model.video_vae.ops import PerChannelStatistics, patchify, unpatchify
from musubi_tuner.ltx_2.model.video_vae.transformer import (
    AdaLNZero,
    ChannelLinear,
...
from musubi_tuner.ltx_2.model.video_vae.transformer.config import DiffVAEMode
from musubi_tuner.ltx_2.tiling import (
    Tile,
    TileSizeConfig,
    TilingConfig,
...
class DiffusionVideoDecoder(nn.Module, Disposable):
    ...
    def __init__(  # noqa: PLR0913
        ...
        for stage_i in range(n_det_stages):
            ...
            self.det_stages.append(
                nn.ModuleList(
                    [NABlock(dim=c, kernel_size=kernel, head_dim=head_dim, rope_dim_split=rope_dim_split) for _ in range(depth)]
                )
            )
            stride, reduction = upsamples[stage_i]
            self.upsamples.append(LinearPixelShuffleUpsample(in_channels=c, stride=stride, out_channels_reduction_factor=reduction))
    ...
    def forward_diff_step_deferred(
        ...
        *,
        ...
        from musubi_tuner.ltx_2.model.video_vae.transformer.dsl_kernels import DSLDiffusionBlockChain  # noqa: PLC0415
        ...
        *,
        ...
        for i in range(num_steps - 1):
            ...
            if self.deferred_stage4_upsample:
                ...
                model_out = self.forward_diff_step_deferred(x, context_tile, t_now, drop_leading_frame=is_origin).to(torch.float32)
            else:
    ...
    def _decode_pixels(  # noqa: PLR0912, PLR0915
        ...
        *,
        ...
        latent_padded = diffusion_tiling.pad_trailing_latent_for_natten_border(latent, self._natten_trailing_pad_latent_frames)
        if self.mark_dynamic_shapes:
            for dim in (2, 3, 4):
        ...
        if not single_step_x0:
            ...
            x_t_init = torch.randn(tuple(target_shape), dtype=compute_dtype, generator=generator, device=randn_device).to(
                latent.device
            )
