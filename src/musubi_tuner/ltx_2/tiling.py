...
from musubi_tuner.ltx_2.types import SpatioTemporalScaleFactors, VideoPixelShape
...
def split_by_count_temporal_causal(num_tiles: int, overlap: int = 0, min_tile_size: int | None = None) -> SplitOperation:
...
def split_by_count(num_tiles: int, overlap: int = 0, min_tile_size: int | None = None) -> SplitOperation:
    ...
    def split(dim_size: int) -> DimensionIntervals:
        if num_tiles > dim_size:
            raise ValueError(f"num_tiles ({num_tiles}) exceeds dim_size ({dim_size}). Cannot assign at least 1 unit per tile.")
        if num_tiles == 1:
        ...
        if tile_size <= overlap:
            raise ValueError(
                f"split_by_count produced size={tile_size} <= overlap={overlap} for dim_size={dim_size}, num_tiles={num_tiles}"
            )
    ...
    *,
    ...
    for tile in tiles:
        if len(tile.out_coords) != ndim or len(tile.masks_1d) != ndim:
            raise ValueError(f"Tile out_coords/masks_1d rank {len(tile.out_coords)}/{len(tile.masks_1d)} != full_shape rank {ndim}")
