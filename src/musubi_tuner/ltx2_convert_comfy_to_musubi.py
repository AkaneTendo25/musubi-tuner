...
def is_ltx2_comfy_adapter_state_dict(weights_sd: Dict[str, torch.Tensor]) -> bool:
    if not weights_sd:
        return False
    return any(
        key.startswith("diffusion_model.") and any(key.endswith(f".{suffix}") for suffix in _KNOWN_COMFY_SUFFIXES + _UNSUPPORTED_COMFY_SUFFIXES)
...
def _reconstruct_musubi_magnitude(
    ...
    if "lora_down.weight" not in module_state or "lora_up.weight" not in module_state:
        raise ValueError(
            f"Unsupported DoRA module '{module_name}': expected native LoRA A/B or native LoKr tensors"
        )
...
def _infer_network_metadata(
    ...
    has_lora = any("lora_down.weight" in module_state or "lora_up.weight" in module_state for module_state in grouped_modules.values())
...
def _grouped_modules_need_base_model(grouped_modules: Dict[str, Dict[str, torch.Tensor]]) -> bool:
    return any(
        "dora_scale" in module_state and "oft_R.weight" not in module_state
        for module_state in grouped_modules.values()
    )
...
def convert_ltx2_comfy_to_musubi_state_dict(
    ...
    *,
    ...
    for module_name, module_state in grouped_modules.items():
        ...
        if "dora_scale" in module_state and "oft_R.weight" not in module_state:
